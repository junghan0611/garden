...
이름은 **카테고리 레코드**. schema.org `ItemList` sidecar. Quartz/`Head.tsx`/`validate-jsonld.mjs`는 안 건드린다.

- 생성기: `scripts/generate-category-records.mjs` (옛 `generate-autholog-itemlist.mjs` 삭제).
- 독립 검증기: `scripts/validate-category-records.mjs` — 각 항목을 public HTML `#article`과 exact compare. 생성기 self-check 아님.
- 산출: `/tags/autholog.jsonld` + `/notes.jsonld` `/meta.jsonld` `/bib.jsonld` `/botlog.jsonld` `/journal.jsonld`. collection `@id` = 인간 HTML 페이지 + `#itemlist`.
...
- 태그 JSON-LD / SKOS / `/tags/` 민속분류를 API로 정규화
- Datasette·쿼리 가능 가든, ItemList SEO, 페이지별 `.json`, Head 인라인, size hard cap
- category validator를 HTML drift에 맞춰 약화하는 것
