...
1. 1단계 셀렉터를 구현한다(폴더+태그 판정, 예외 목록 없음). 두 미러가 공유할 코드다.
2. 블로그스팟 발행 경로를 정하고 264개를 올린다 (Blogger API / 수동 / 별도 하네스 — 미정).
3. 그 뒤에 위키독스 축소를 설계한다. **결정 필요**: 기존 page_id를 삭제할지 비공개로 돌릴지,
