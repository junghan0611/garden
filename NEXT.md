...
- **Next**: after deploy → **manual validator.schema.org + Google Rich Results** on live home + one note URL (browser). **Watch the `meta` multi-type `["Article","DefinedTerm"]` warning** — if validator warns, roll back `meta` to single `"Article"` (1-line). #1 refs awaits dexport schema notification.
...
3. ~~**Path-based schema type mapping**~~ — **DONE in code 2026-06-29** (`Head.tsx` `typeBySection`): `notes→Article`, `botlog→TechArticle`, `bib`·`journal→CreativeWork`, `meta→["Article","DefinedTerm"]` (multi-type), unknown→`BlogPosting` fallback. `@id` suffix kept `#article` for crawler-merge stability. Build-verified (Claude + GPT independent): Article 837 / TechArticle 80 / CreativeWork 781 / Article+DefinedTerm 538 / fallback 0, parse-fail 0. **meta multi-type = 1st rollback候補 if validator.schema.org warns → `meta:"Article"` single.** Pending GLG build/deploy + manual validator.
...
5. ~~**BreadcrumbList JSON-LD**~~ — **DONE in code 2026-06-29** (`Head.tsx`): 3-level Home→section→page, `#breadcrumb` node + content node `breadcrumb` ref. Section item `/<sec>/` resolves (FolderPage active, all 5 section index pages exist). Build-verified BreadcrumbList 2236.
