...
<style>.csl-entry{text-indent: -1.5em; margin-left: 1.5em;}</style><div class="csl-bib-body">
  <div class="csl-entry">짐 웨버. 2021. <i>Knowledge Graphs 지식그래프</i>. <a href="https://www.oreilly.com/library/view/knowledge-graphs/9781098104863/">https://www.oreilly.com/library/view/knowledge-graphs/9781098104863/</a>.</div>
  <div class="csl-entry">짐 웨버, and Barrasa Jesus. 2023. <i>Building Knowledge Graphs 지식그래프 구축 - 실무자 가이드</i>. 1st edition. O’Reilly Media.</div>
...
-   [@젠스알브레히트 파이썬 텍스트분석 지식그래프 (2024-06-04)]({{< relref "/bib/20240604T121824.md" >}})
...
## Knowledge Graphs 지식그래프 {#knowledge-graphs-지식그래프}
...
## Building Knowledge Graphs 지식그래프 구축 - 실무자 가이드 {#building-knowledge-graphs-지식그래프-구축-실무자-가이드}
...
Knowledge graphs are a type of graph, so it's important to have a basic understanding of graphs. Graphs are simple structures that use nodes (or vertices) connected by relationships (or edges) to create high-fidelity models of a domain. To avoid any confusion, the graphs in this book have nothing to do with visualizing data as histograms or plotting a function, which are called _charts_, as shown in [BROKEN LINK: ch01.html_graphs-vs-charts].
...
-  Note
    ...
    You can use the superimposed graph in [BROKEN LINK: ch01.html_town-of-konigsberg] to figure out the shortest route for walking around Königsberg without having to put on your walking boots and try it for real. In fact, Euler proved that the emperor could _not_ walk the whole town crossing each bridge only once, since there would have needed to be (at least) one island (node) with an even number of connecting bridges (relationships) from which the emperor could start his walk. No such island existed in Königsberg, so no such route (path) is possible.
...
[BROKEN LINK: ch01.html_small-social-graph] shows a small social graph, but compared to the example in [BROKEN LINK: ch01.html_town-of-konigsberg], this graph holds much more information.
...
-  Tip
    ...
    Between the nodes in [BROKEN LINK: ch01.html_small-social-graph] you see relationships. The relationships are richer than in [BROKEN LINK: ch01.html_town-of-konigsberg] since they have a type, a direction, and optional properties. Relationships cannot “dangle”; they always have a start node and an end node, even if the start and end nodes are the same.
...
From here on things get a little more technical. In [BROKEN LINK: ch02.html#ch02] you'll learn how we can extend the definition of an organizing principle to act as a contract between a knowledge graph and its consuming users and systems. You'll also learn about several different options for creating organizing principles.
