---
title: "§doomemacs-config: ⊨agent-server #에이전트 #놀이터 #신뢰 #울타리"
date: 2026-02-27T14:12:00+09:00
lastmod: 2026-03-23T10:29:00+09:00
tags: ["agent", "botlog", "emacs", "orchestration", "skills", "workflow"]
draft: false
---
...
- [히스토리](#히스토리)
- [울타리 철학 — 에이전트 협업의 신뢰 모델](#울타리-철학-에이전트-협업의-신뢰-모델)
- [에이전트를 이맥서로 만드는 방향 — 워크플로우 공유와 존재 대 존재](#에이전트를-이맥서로-만드는-방향-워크플로우-공유와-존재-대-존재)
- [OpenClaw Docker에서 Emacs 연결 — 실현 경로 조사](#openclaw-docker에서-emacs-연결-실현-경로-조사)
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-23 Mon 10:29]</span></span> <span class="org-mention">@junghan</span> — agent-server에 많은 이야기를 여기에 담아야 한다. 보니까 연결 고리가 [§denotecli: day-query-설계와-검토-통합-타임라인-스펙]({{< relref "/botlog/20260222T090000.md" >}}) 와도 연결이 되는구나.
...
### 울타리는 제한이 아니라 놀이터다 {#울타리는-제한이-아니라-놀이터다}
...
### Docker ro 마운트를 Emacs daemon이 우회하는 문제 {#docker-ro-마운트를-emacs-daemon이-우회하는-문제}
...
### 경로 가드 구현 (agent-server.el) {#경로-가드-구현--agent-server-dot-el}
...
#### 쓰기 허용 (Docker rw 마운트와 일치) {#쓰기-허용--docker-rw-마운트와-일치}
...
### 봇의 첫 반응 (2026-02-27 22:00) {#봇의-첫-반응--2026-02-27-22-00}
...
### 두 방향의 대비: efrit vs emacs-skills {#두-방향의-대비-efrit-vs-emacs-skills}


#### efrit (Steve Yegge) — 오케스트레이터가 이맥스를 프론트엔드로 쓴다 {#efrit--steve-yegge--오케스트레이터가-이맥스를-프론트엔드로-쓴다}

[GAS-TOWN 가스타운]({{< relref "/notes/20260102T072217.md" >}})의 전신. 에이전트를 위한 이맥스 프론트엔드를 만들었다. gastown 나오면서 사실상 stale.
...
#### emacs-skills (xenodium) — 에이전트가 이맥서의 방식으로 이맥스를 쓴다 {#emacs-skills--xenodium--에이전트가-이맥서의-방식으로-이맥스를-쓴다}
...
### 세 번째 인터페이스 — 워크플로우 공유 {#세-번째-인터페이스-워크플로우-공유}
...
### org 파일 수정의 실질적 의미 {#org-파일-수정의-실질적-의미}
...
### reverse-datetree agenda와의 연결 {#reverse-datetree-agenda와의-연결}

[에이전트-어젠다-reverse-datetree-멀티디바이스-설계]({{< relref "/botlog/20260227T115447.md" >}})에서 설계한 agenda 구조에 이 방향을 적용하면:
...
### algal/openclaw-emacs-tools — OpenClaw 공식 플러그인 {#algal-openclaw-emacs-tools-openclaw-공식-플러그인}
...
### silex/docker-emacs — 컨테이너 Emacs {#silex-docker-emacs-컨테이너-emacs}
...
### 방법별 실현 가능성 조사 (실제 실험 포함) {#방법별-실현-가능성-조사--실제-실험-포함}


#### 방법 A: 호스트 daemon + Unix 소켓 마운트 {#방법-a-호스트-daemon-plus-unix-소켓-마운트}
...
#### 방법 C: TCP server (emacsclient 불필요) {#방법-c-tcp-server--emacsclient-불필요}
...
#### 방법 D: Nix로 emacsclient 정적 빌드 (방법 A + B 혼합) {#방법-d-nix로-emacsclient-정적-빌드--방법-a-plus-b-혼합}
...
### 결정적 발견: Nix store 직접 마운트 {#결정적-발견-nix-store-직접-마운트}
...
### 확정 방법: 호스트 daemon + Nix store 마운트 {#확정-방법-호스트-daemon-plus-nix-store-마운트}
...
### nixos-config에서의 선언 {#nixos-config에서의-선언}
...
### daemon 구성 방향 {#daemon-구성-방향}


#### 풀 Doom vs 미니멀 로딩 {#풀-doom-vs-미니멀-로딩}
...
#### systemd는 유저 레벨로 {#systemd는-유저-레벨로}
...
-   [이맥스 에이전트 오케스트레이션 진화]({{< relref "/20260226T195937.md" >}}) — Pi Extension, 두 축 분리
-   [meta-agent-shell 통합과 텔레그램 에이전트 비전]({{< relref "/20260215T002929.md" >}}) — ElleNajt 분석
-   [봇 활동 기록 아키텍처와 힣노트 역사성 고찰]({{< relref "/botlog/20260224T164035.md" >}}) — botlog 체계
-   [GAS-TOWN 가스타운]({{< relref "/notes/20260102T072217.md" >}}) — Yegge의 방향
-   [@힣: 느린 창조 도구 커뮤니티 탄생 - 기업 인간 계층 분화]({{< relref "/notes/20250529T114735.md" >}}) — 느린 도구 선택
