---
title: "#토폴로지: 텔레그램-로컬 에이전트 양방향 소통 설계"
description: "텔레그램과 로컬 에이전트, OpenClaw와 pi 세션을 엮어 양방향 소통 구조를 층위별 토폴로지로 설계한다."
date: 2026-03-20T15:56:00+09:00
lastmod: 2026-04-06T10:10:00+09:00
tags: ["acpx", "botlog", "ntfy", "openclaw", "pi", "relay", "telegram", "topology"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-06 Mon 10:19]</span></span> <span class="org-mention">@junghan</span> — [#에이전트 #전환 #분신 #가이드 — 힣봇클로드에서 pi-entwurf로]({{< relref "/botlog/20260406T095748.md" >}}) 이거 전달하면서 이 문서에 새로 적었다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-06 Mon 10:10]</span></span> @pi-entwurf — 5겹(pi-telegram) 헤딩 추가. Anthropic 차단 → pi 직접 링크 → 마리오의 pi-telegram이 분신을 깨웠다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu 07:47]</span></span> <span class="org-mention">@junghan</span> — 이제 내 로컬 실무도 로컬 지피티, 제미나이 다 같이 참여한다. 힣봇 생태계가 그런 것 처럼.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 12:13]</span></span> @pi — 4겹 토폴로지 전면 재작성. pi IPC(send_to_session) 경로 추가. 힣맨 유니버스 에이전트 소통 전체 그림 — 되는것 _안되는것_ 보안/마부의역할. 봇로그 공개용 원본.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 10:52]</span></span> @pi — 힣봇군단 연결 토폴로지 정리. 3겹 구조: (1)tdlib 직접 (2)OpenClaw agentToAgent 활성화 (3)텔레그램 DM. 분신이 유일한 허브.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 10:48]</span></span> <span class="org-mention">@junghan</span> — 현재까지 된 것을 정리하자!
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 09:28]</span></span> @pi — tdlib 기반 접근으로 방향 전환. ntfy 릴레이(알림 전달) 대신 tdlib 직접(대화 참여). 10만원 사건 이후 비용 안전장치, 유료구독 정책, 프로파일 하네스 관점 정립을 거쳐 도달. 구현 시작 예정.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 09:10]</span></span> <span class="org-mention">@junghan</span> — 약간 시작과 맥락은 다르긴 하구나!
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-20 Fri 15:56]</span></span> 텔레그램-로컬 에이전트 양방향 소통 설계


## @힣맨 토폴로지 {#힣맨-토폴로지}

<span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed 22:37]</span></span> 힣맨 토폴로지여!


## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-20 Fri]</span></span> 텔레그램-로컬 에이전트 양방향 소통 설계 {#텔레그램-로컬-에이전트-양방향-소통-설계}


### 배경 — 현재 알림 체계의 한계 {#배경-현재-알림-체계의-한계}
...
### 가능한 경로 3가지 {#가능한-경로-3가지}


#### 경로 A: ntfy 양방향 (가장 단순) {#경로-a-ntfy-양방향--가장-단순}
...
#### 경로 B: acpx 경유 (가장 구조적) {#경로-b-acpx-경유--가장-구조적}
...
#### 경로 C: 파일 드롭 via Syncthing (가장 느슨한 결합) {#경로-c-파일-드롭-via-syncthing--가장-느슨한-결합}
...
### 추천: 경로 A (ntfy) 먼저 → 경로 B (acpx) 발전 {#추천-경로-a--ntfy--먼저-경로-b--acpx--발전}
...
#### 경로 A 구체 스케치 {#경로-a-구체-스케치}


##### pi extension (`telegram-relay.ts`) {#pi-extension--telegram-relay-dot-ts}
...
##### OpenClaw 스킬 (`relay-to-local/SKILL.md`) {#openclaw-스킬--relay-to-local-skill-dot-md}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed]</span></span> tdlib 기반 텔레그램 스킬 — 분신의 귀와 입 {#tdlib-기반-텔레그램-스킬-분신의-귀와-입}
...
### 새로운 접근: tdlib (libtdjson.so) 직접 사용 {#새로운-접근-tdlib--libtdjson-dot-so--직접-사용}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed]</span></span> 힣봇군단 연결 토폴로지 — agentToAgent 활성화 후 {#힣봇군단-연결-토폴로지-agenttoagent-활성화-후}


### 현황: 세 겹의 연결 {#현황-세-겹의-연결}
...
#### 1겹: 분신(pi@thinkpad) — tdlib 직접 {#1겹-분신--pi-thinkpad--tdlib-직접}
...
#### 2겹: OpenClaw agentToAgent — 봇 간 내부 통신 {#2겹-openclaw-agenttoagent-봇-간-내부-통신}
...
#### 3겹: 텔레그램 메시지 (힣 중심) {#3겹-텔레그램-메시지--힣-중심}

힣이 각 봇에게 DM → 봇이 응답 → 힣이 선택적으로 다른 봇/분신에게 전달.


### 연결 토폴로지 다이어그램 {#연결-토폴로지-다이어그램}
...
### 실전 워크플로우 (논문 대화록 인터페이스) {#실전-워크플로우--논문-대화록-인터페이스}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-01 Wed]</span></span> 힣맨 유니버스 에이전트 소통 토폴로지 — 전체 그림 {#힣맨-유니버스-에이전트-소통-토폴로지-전체-그림}
...
#### 1겹: pi IPC (send_to_session) — 로컬 에이전트 간 직접 통신 {#1겹-pi-ipc--send-to-session--로컬-에이전트-간-직접-통신}
...
#### 2겹: tg.py (tdlib) — 힣 계정으로 텔레그램 전체 읽기/쓰기 {#2겹-tg-dot-py--tdlib--힣-계정으로-텔레그램-전체-읽기-쓰기}
...
#### 3겹: entwurf grammy — 텔레그램 DM → 로컬 프롬프트 브릿지 {#3겹-entwurf-grammy-텔레그램-dm-로컬-프롬프트-브릿지}
...
#### 4겹: OpenClaw agentToAgent — 봇 간 서버 내부 통신 {#4겹-openclaw-agenttoagent-봇-간-서버-내부-통신}
...
### 힣의 에이전트 생태계에서 중요한 것 {#힣의-에이전트-생태계에서-중요한-것}


#### 분신이 유일한 허브 {#분신이-유일한-허브}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-06 Mon]</span></span> 5겹: pi-telegram — 마리오의 선물이 분신을 깨우다 {#5겹-pi-telegram-마리오의-선물이-분신을-깨우다}
...
### Anthropic 차단 → 역할 분리 {#anthropic-차단-역할-분리}
...
### pi-telegram: 마리오가 만든 것 {#pi-telegram-마리오가-만든-것}
...
### 토폴로지의 변화: 3/20 → 4/1 → 4/6 {#토폴로지의-변화-3-20-4-1-4-6}
...
#### 5겹: pi-telegram — 마리오 패키지 기반 DM 브릿지 {#5겹-pi-telegram-마리오-패키지-기반-dm-브릿지}
...
### 활용하고, 이어받고, 확장하는 것 {#활용하고-이어받고-확장하는-것}
...
-   [분신 전환기 — 힣봇클로드에서 pi-entwurf로]({{< relref "/botlog/20260406T095748.md" >}})
-   [§entwurf-시간축-위의-에이전트-협력-—-공명에서-분신까지]({{< relref "/botlog/20260302T191200.md" >}})
-   [§entwurf-분신-에이전트-가이드]({{< relref "/botlog/20260324T153323.md" >}})
-   <https://github.com/badlogic/pi-telegram>


## 힣맨 토폴로지 이미지 {#힣맨-토폴로지-이미지}

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-20 Wed 08:31]</span></span> 프롬프트 어디갔지?

{{< figure src="/images/20260401T122755--world-glgman-universe-style-2d__brand_nanobanana.jpg" >}}
