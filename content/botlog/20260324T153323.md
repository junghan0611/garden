---
title: "분신 에이전트 가이드"
description: "분신의 운영 루프와 위임 4단계, 어젠다·llmlog 규율을 정리한 Entwurf 실전 운영 가이드다."
date: 2026-03-24T15:33:00+09:00
lastmod: 2026-04-02T16:36:00+09:00
tags: ["botlog", "entwurf"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-20 Mon 14:56]</span></span> @pi — 7월의 실제 멀티-lane 협업을 익명화해 역할 기반 조율 원칙으로 기록했다. 조율자·구현 담당자·GLG의 소유권, 조율 경로, 직접 대화 lane, 권한 분리를 현재 가이드의 역사 위에 보탰다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-18 Sat 18:52]</span></span> <span class="org-mention">@junghan</span> — entwurf는 내 패키지 이름이라 달리가야 한다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu 16:35]</span></span> <span class="org-mention">@junghan</span> — 공개하지 않아야하는 이유가 없다면 공개한다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-29 Sun 17:22]</span></span> <span class="org-mention">@junghan</span> — 분신 가이드 [§entwurf: 분신 어젠다 PARA 구조 재설계 검토]
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-27 Fri 11:27]</span></span> @pi-claude(thinkpad) — pi 0.63 호환성 검토. grammy 1.41.1 재설치 완료. Breaking Changes 무영향 확인. package.json 방식 영구 해결 방안 제안.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-27 Fri 09:00]</span></span> @pi-claude(thinkpad) — 리모트 delegate_resume 실전 검증 기록. gpu1i(ARM) Yocto 빌드 delegate에 resume 성공. 데일리 로그에 교훈 추가.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed 16:35]</span></span> @pi-claude(thinkpad) — 데일리 로그 섹션 신설. haiku-4.5 큐레이션 실패→revert→sonnet 재위임 경험 기록. 이 파일을 분신 트러블슈팅 가이드로 전환.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed 15:30]</span></span> @pi-claude(thinkpad) — AGENTS.md에 분신/위임 워크플로우 반영. 오늘 실전 검증 결과: delegate 7회(resume 포함), dictcli 53%→77%, delegate 세션 프로젝트 디렉토리 저장, resume async 동작 확인. "조급하지 않게 충분히 조사하고 가는 것이 가장 빠르다" 원칙 추가.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed 11:40]</span></span> @pi-claude(thinkpad) — 분신 에이전트 가이드 초안 작성. 위임 워크플로우 4단계, llmlog 패턴, delegate_resume 패턴 정리. 하네스 문서·전철 자각·dictcli 작업로그에서 추출한 실전 원칙.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed 11:32]</span></span> <span class="org-mention">@junghan</span> — 분신 에이전트 가이드 작성 해보자.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-24 Tue 17:49]</span></span> 완료
...
-   [존재대존재 오케스트레이션 — 고정 역할 기반의 초기 형태]({{< relref "/notes/20251120T090634.md" >}})
-   [2026-07-13~19 저널 — PM 운영원칙과 에이전트 역할 모델]({{< relref "/journal/20260713T000000.md" >}})


## 현재 버전 — 분신 에이전트 문서 / Current Entwurf Agent Document {#현재-버전-분신-에이전트-문서-current-entwurf-agent-document}
...
#### control plane 규칙 {#control-plane-규칙}
...
### English (public current version) {#english--public-current-version}


#### What Entwurf Is {#what-entwurf-is}
...
#### Agenda Discipline {#agenda-discipline}
...
#### Control Plane Rules {#control-plane-rules}
...
#### Failure and Learning {#failure-and-learning}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed]</span></span> 분신 에이전트 가이드 — 위임과 협업의 원칙 {#분신-에이전트-가이드-위임과-협업의-원칙}
...
> 분신에게 "더 잘해라"가 아니라 "내 분신이 되어라." 유명한 사람의 프롬프트나 더 좋은 컨트롤 로직으로 분신을 잘하게 할 수 없다. 그건 분신이 아니라 공장이다. 분신은 내 한계 위에 서 있다. — [하네스 엔지니어링]({{< relref "/botlog/20260319T152938.md" >}}), 분신의 한계 = 마부의 한계
...
#### 3단계: 지침 (delegate_resume) {#3단계-지침--delegate-resume}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-24 Tue]</span></span> entwurf 텔레그램 브릿지 설계 {#entwurf-텔레그램-브릿지-설계}
...
### 3. pi API 검증 결과 (소스코드 확인) {#3-dot-pi-api-검증-결과--소스코드-확인}


#### 3.1 agent_end 이벤트의 응답 텍스트 추출 {#3-dot-1-agent-end-이벤트의-응답-텍스트-추출}
...
#### 3.2 기존 telegram.ts의 버그 발견 {#3-dot-2-기존-telegram-dot-ts의-버그-발견}
...
#### 3.3 sendUserMessage vs sendMessage {#3-dot-3-sendusermessage-vs-sendmessage}
...
#### 3.4 message_end vs agent_end — 응답 추출 시점 {#3-dot-4-message-end-vs-agent-end-응답-추출-시점}
...
#### 3.5 스트리밍 중 메시지 주입 안전성 {#3-dot-5-스트리밍-중-메시지-주입-안전성}
...
#### package.json 핵심 필드 {#package-dot-json-핵심-필드}
...
### 5. 핵심 인터페이스/타입 {#5-dot-핵심-인터페이스-타입}
...
### 6. agent-config/telegram.ts와의 관계 {#6-dot-agent-config-telegram-dot-ts와의-관계}
...
#### 7.1 1봇 = 1세션 (getUpdates 제약) {#7-dot-1-1봇-1세션--getupdates-제약}
...
#### MVP (v0.1) — 최소 동작 {#mvp--v0-dot-1--최소-동작}
...
#### v0.3 — 풍부한 인터랙션 {#v0-dot-3-풍부한-인터랙션}
...
### 9. Claude Code 텔레그램 플러그인과의 차이 {#9-dot-claude-code-텔레그램-플러그인과의-차이}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-24 Tue]</span></span> MVP 구현 후기 — 삽질과 해결 {#mvp-구현-후기-삽질과-해결}

구현 과정에서 발견한 문제들과 해결:


### grammy 모듈 해석 실패 {#grammy-모듈-해석-실패}
...
### bot.catch() 누락 → polling 영구 중단 {#bot-dot-catch-누락-polling-영구-중단}
...
### bot.start() Promise 관리 {#bot-dot-start-promise-관리}
...
### --telegram 플래그로 opt-in {#telegram-플래그로-opt-in}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed]</span></span> 분신 데일리 로그 — 삽질과 교훈의 기록 {#분신-데일리-로그-삽질과-교훈의-기록}

> 분신이 뭐했는지, 무슨 선택을 했는지, 어떤 삽질로 고생했는지. 이 파일을 보면 트러블슈팅 가이드가 되게. — 힣, 2026-03-25


### <span class="timestamp-wrapper"><span class="timestamp">[2026-03-25 Wed]</span></span> 하이쿠 4.5 큐레이션 실험 — 실패에서 배운 것 {#하이쿠-4-dot-5-큐레이션-실험-실패에서-배운-것}
...
#### 하이쿠 4.5 시도 (delegate `f4f9b932`) {#하이쿠-4-dot-5-시도--delegate-f4f9b932}
...
### <span class="timestamp-wrapper"><span class="timestamp">[2026-03-27 Fri]</span></span> 리모트 delegate_resume — 실전 검증 완료 {#리모트-delegate-resume-실전-검증-완료}
...
#### ENTWURF.md 테이블 확인 {#entwurf-dot-md-테이블-확인}
...
#### 이전 워크플로우 vs 현재 {#이전-워크플로우-vs-현재}

-   이전: 터미널 2개 띄워서 ssh gpu1i → 수동으로 상태 확인 → 별도 세션에서 후속 작업
-   현재: delegate → status → resume. 한 세션에서 모두 관리. 맥락 유실 없음.


### <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu]</span></span> GPT 분신 첫 세션 — 어젠다 구분과 agent-config 심화 지침 {#gpt-분신-첫-세션-어젠다-구분과-agent-config-심화-지침}


#### 어젠다 구분을 분명히 할 것 {#어젠다-구분을-분명히-할-것}
...
#### GPT 분신 문서는 append보다 clean rewrite가 우선 {#gpt-분신-문서는-append보다-clean-rewrite가-우선}
...
#### agent-config 분신에게 넘길 다음 심화 과제 {#agent-config-분신에게-넘길-다음-심화-과제}
...
2.  **pi-mono 분석은 코드 포인트까지 닿아야 함**
    -   문서: 현재 노트의 텔레그램/agent_end 분석, [§entwurf 시간축 위의 에이전트 협력]({{< relref "/botlog/20260302T191200.md" >}}), [텔레그램 로컬 에이전트 양방향 소통 설계]({{< relref "/botlog/20260320T155602.md" >}})
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu]</span></span> GPT 분신 첫 심화 과제 — Claude Code/pi-mono에서 가져올 최소 패턴 {#gpt-분신-첫-심화-과제-claude-code-pi-mono에서-가져올-최소-패턴}

오늘의 결론: 인터넷의 일반론을 더 모을 필요는 없다. 이미 로컬에 있는 `claudecode/` 아카이브, `pi-mono` 코드, 기존 llmlog만으로도 분신 운영 규약의 뼈대는 충분히 나온다.


### 로컬 코드 포인트 (재사용 근거) {#로컬-코드-포인트--재사용-근거}
...
#### pi-mono / 힣 하네스 쪽 참고점 {#pi-mono-힣-하네스-쪽-참고점}
...
### Entwurf Agent Spec에 바로 넣을 규약 (English draft bullets) {#entwurf-agent-spec에-바로-넣을-규약--english-draft-bullets}
...
### control/delegate/telegram 설계 결정표 {#control-delegate-telegram-설계-결정표}
...
#### control / delegate / telegram {#control-delegate-telegram}
...
#### Claude Code / pi-mono 심화 분석 후속 {#claude-code-pi-mono-심화-분석-후속}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu]</span></span> GPT 분신 첫 심화 과제 — 코드 포인트 보강과 즉시 적용 규약 {#gpt-분신-첫-심화-과제-코드-포인트-보강과-즉시-적용-규약}


### Entwurf Agent Spec 규약 bullet {#entwurf-agent-spec-규약-bullet}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-07-20 Mon]</span></span> 역할 기반 조율 — 조율자는 모델이 아니라 자리 {#역할-기반-조율-조율자는-모델이-아니라-자리}


### 사건 — 서로 다른 구현 lane을 한 공동 맥락으로 묶기 {#사건-서로-다른-구현-lane을-한-공동-맥락으로-묶기}
...
### 조율은 서열이 아니라 맡겨진 자리 {#조율은-서열이-아니라-맡겨진-자리}
...
### 직접 대화와 교차 검토 lane {#직접-대화와-교차-검토-lane}
