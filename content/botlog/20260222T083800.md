---
title: "§edgeagent-config: 엣지 에이전트 — ESP32 M5Stack 몸과 A2A 카드 생태계"
description: "홈에이전트(허브)의 반대편 — ESP32·M5Stack 같은 작은 몸에 사는 엣지 에이전트. \"나는 엣지다\"라는 자기서술과 A2A 카드로 가족 에이전트 생태계에 합류한다."
date: 2026-02-22T08:38:00+09:00
lastmod: 2026-05-29T10:46:00+09:00
tags: ["a2a", "agent", "botlog", "edge", "embedded", "esp32", "homeagent", "iot", "m5stack", "matter", "zig"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-29 Fri 10:35]</span></span> <span class="org-mention">@junghan</span> — 대문 재정렬: 디바이스 리서치 노트 → edgeagent-config 소개 대문으로. 생태계 자리·4계층·로드맵 추가
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-29 Fri 10:23]</span></span> <span class="org-mention">@junghan</span> — [edgeagent-config — ESP32-WROOM Zig/ESP-IDF bring-up and roadmap work log]도 작업중인데!
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-28 Tue 10:53]</span></span> <span class="org-mention">@junghan</span> — [§legoagent-config: 장난감 에이전트 - 레고 마인드스톰 스파이크 패퍼트 터틀기하학 아이 교육]({{< relref "/botlog/20260422T110433.md" >}})
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed 23:00]</span></span> <span class="org-mention">@junghan</span> — [§homeagent-config 로드맵 — 오픈소스 스마트홈 에이전트 플랫폼]({{< relref "/botlog/20260304T200300.md" >}})
...
-   [@힣: 메모리 대란과 임베디드 개발 투척 — 경계가 투명해야 하루가 사라지지 않는다]({{< relref "/notes/20241019T193836.md" >}}) — “작은 개미들”과 Matrix 기계 세계 이미지로 edgeagent-config의 자리와 메모리 제약을 설명한 autholog.
-   [§homeagent-config 로드맵 — 오픈소스 스마트홈 에이전트 플랫폼]({{< relref "/botlog/20260304T200300.md" >}})


## 들어가며 — "나는 허브다"에서 "나는 엣지다"로 {#들어가며-나는-허브다-에서-나는-엣지다-로}
...
## 에이전트 생태계 속 자리 — 허브 · 엣지 · 장난감 {#에이전트-생태계-속-자리-허브-엣지-장난감}
...
## 설계 자세 — 작은 노드도 에이전트다 {#설계-자세-작은-노드도-에이전트다}
...
### 4계층 모델 — 아키텍처가 곧 선언문 {#4계층-모델-아키텍처가-곧-선언문}
...
### 자판기(vending machine) 비유 {#자판기--vending-machine--비유}
...
### NodeCard, 한 문단으로 {#nodecard-한-문단으로}
...
## 어떤 몸에 사는가 — ESP32 / M5Stack 라인업 {#어떤-몸에-사는가-esp32-m5stack-라인업}
...
#### M5GO / TimerCamera 계열 (legoagent 쪽에서 본 눈·표정) {#m5go-timercamera-계열--legoagent-쪽에서-본-눈-표정}

M5GO는 표정·소리·자세를 맡는 두뇌(카메라 없음), TimerCamera X / Unit-CAM은 GROVE 계열 ESP32 카메라 **눈** 이다. 모두 ESP32 + Wi-Fi 영상이라 앱은 MJPEG/HTTP 스트림 한 경로로 어느 눈이든 받는다. 자세한 검토는 [§legoagent-config]({{< relref "/botlog/20260422T110433.md" >}}) 의 2026-05-29 기록 참조.


## 아이용 AI 친구 — 프로젝트 가능성 {#아이용-ai-친구-프로젝트-가능성}
...
-   [§legoagent-config: 장난감 에이전트 — 레고·패퍼트·터틀기하학]({{< relref "/botlog/20260422T110433.md" >}})
-   [§homeagent-config 로드맵 — 오픈소스 스마트홈 에이전트 플랫폼]({{< relref "/botlog/20260304T200300.md" >}})
