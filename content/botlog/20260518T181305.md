...
description: "NEXT.md를 다음 세션이 즉시 이어받게 만드는 패턴 묶음 — 검증 기준 내장 + NOW/RECENT/LEDGER 3층 + 프로젝트 유형별 스타일(단독 repo vs 메타리포 포트폴리오) + 브랜치 별동대용 NEXT--<branch>.md + OpenClaw 봇 워크스페이스 NEXT Mesh + NEXT를 버리지 않고 repo에 문서화하는 AGENTS/README/CHANGELOG/ROADMAP 4문서와 tag-release 의식. beads를 버리고 하네스 TODO를 끈 뒤 남긴 최소 공통 표면."
date: 2026-05-18T18:13:00+09:00
lastmod: 2026-06-15T17:25:00+09:00
tags: ["agent", "bookmark", "botlog", "delegation", "entwurf", "handoff", "next", "sessions", "verification", "workflow"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-15 Mon 17:25]</span></span> OpenClaw 봇 워크스페이스 NEXT Mesh 층위 추가 — repo/metarepo 밖에서 각 힣봇 local NEXT와 global nextmesh view를 나누는 패턴을 기록. agent-config#15로 구현 포인터 연결.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-13 Sat 15:35]</span></span> 브랜치 별동대 정책 추가 — main으로 가는 에이전트 중심축을 유지하면서도 브랜치 작업은 `NEXT--<branch>.md` 로 분기해 병렬 검수 /별동대를 조직하는 규칙을 초벌구이로 정리. `~/AGENTS.md` 에 넣은 영어 원문 프롬프트도 재사용 블록으로 수록.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-12 Fri 11:45]</span></span> 프로젝트 유형 축 + 문서화 메커니즘 추가 — 단독 repo(3층) vs 메타리포 /리포의-리포(포트폴리오 대시보드)로 NEXT 스타일이 갈린다는 패턴 3, NEXT를 버리지 않고 repo에 문서화하는 AGENTS/README/CHANGELOG/ROADMAP 4문서 + tag-release 의식(패턴 4)을 cos 비서실장 repo 실적용으로 정립. beads를 버리고 하네스 TODO를 끈 선택의 맥락도 명시. denote id 유지.
...
### 패턴 3 — 프로젝트 유형이 NEXT 스타일을 정한다 (2026-06-12 추가) {#패턴-3-프로젝트-유형이-next-스타일을-정한다--2026-06-12-추가}

같은 NEXT.md라도 **repo가 무엇이냐** 에 따라 모양이 달라야 한다. 공통 기조는 하나다 — NEXT는 부트 섹터, 짧고 안 낡고 첫 줄에 상태가 보인다. 그 위에서 두 갈래가 갈린다.


#### 단독 repo — 한 가지 일을 깊게 (3층 NOW/RECENT/LEDGER) {#단독-repo-한-가지-일을-깊게--3층-now-recent-ledger}
...
#### 메타리포 / 리포의 리포 — 여러 트랙·여러 주체를 한눈에 (포트폴리오 대시보드) {#메타리포-리포의-리포-여러-트랙-여러-주체를-한눈에--포트폴리오-대시보드}
...
### 패턴 4 — NEXT를 버리지 않고 repo에 문서화한다 (2026-06-12 추가) {#패턴-4-next를-버리지-않고-repo에-문서화한다--2026-06-12-추가}
...
#### tag-release 가 그 "내리는 의식"이다 {#tag-release-가-그-내리는-의식-이다}
...
### 패턴 5 — 브랜치 별동대 NEXT: `NEXT--<branch>.md` (2026-06-13 추가) {#패턴-5-브랜치-별동대-next-next-branch-dot-md--2026-06-13-추가}
...
#### 왜 worktree가 아니라 파일명인가 {#왜-worktree가-아니라-파일명인가}
...
#### 글로벌 AGENTS.md에 넣은 영어 원문 {#글로벌-agents-dot-md에-넣은-영어-원문}
...
### 패턴 6 — OpenClaw 봇 워크스페이스 NEXT Mesh (2026-06-15 추가) {#패턴-6-openclaw-봇-워크스페이스-next-mesh--2026-06-15-추가}
...
### 왜 지금 이 방법인가 — beads를 버리고, 하네스 TODO를 끄고 (2026-06-12) {#왜-지금-이-방법인가-beads를-버리고-하네스-todo를-끄고--2026-06-12}
