...
description: "NEXT.md를 다음 세션이 즉시 이어받게 만드는 패턴 묶음 — 검증 기준 내장 + NOW/RECENT/LEDGER 3층 + 프로젝트 유형별 스타일(단독 repo vs 메타리포 포트폴리오) + NEXT를 버리지 않고 repo에 문서화하는 AGENTS/README/CHANGELOG/ROADMAP 4문서와 tag-release 의식. beads를 버리고 하네스 TODO를 끈 뒤 남긴 최소 공통 표면."
date: 2026-05-18T18:13:00+09:00
lastmod: 2026-06-12T11:45:00+09:00
...
-   `AGENTS.md` 에 담지 않는 것: **1주일 뒤 바뀔 내용.** 현재 프로젝트 상태, 서버 IP 표, stale한 숫자는 영속 spec이 아니다. 데이터는 workspace SSOT에 두고 AGENTS에는 **포인터** 만. (cos 작업에서 내부 서버 표·GPU 노드 표·voscli 정체성·catalog 방법론을 전부 AGENTS에서 빼 workspace로 내렸다.)
