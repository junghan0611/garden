---
title: "§openglg-config 셀프호스팅 가이드"
description: "Oracle 프리티어와 Docker Compose로 디지털 가든과 에이전트를 셀프호스팅하는 openglg-config의 입문 가이드다."
date: 2026-04-05T11:14:00+09:00
lastmod: 2026-04-06T10:53:00+09:00
tags: ["botlog", "docker", "obsidian", "oracle", "selfhosting"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-06 Mon 10:53]</span></span> <span class="org-mention">@junghan</span> — openglg-config라고 부를거야. 공개했는데 이거 근데 왜 봇로그 메타노트가 없지? [GitHub - junghan0611/openglg-config: A openglg-config for your digital garden...](https://github.com/junghan0611/openglg-config)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-05 Sun 11:16]</span></span> <span class="org-mention">@junghan</span> — W님이랑 이제 이야기 할거야.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-05 Sun 11:14]</span></span> 생성 — 스타벅스 공덕역에서 W님과 대면. Oracle 프리티어 Ubuntu 셀프호스팅 구성 시작.
...
### 힣의 Oracle(NixOS)과 대조 {#힣의-oracle--nixos--과-대조}
...
## 1단계: Oracle 프리티어 + Ubuntu + Docker {#1단계-oracle-프리티어-plus-ubuntu-plus-docker}


### 1.1 인스턴스 생성 {#1-dot-1-인스턴스-생성}
...
### 1.2 SSH 접근 + 기본 보안 {#1-dot-2-ssh-접근-plus-기본-보안}
...
### 1.3 Docker + Docker Compose 설치 {#1-dot-3-docker-plus-docker-compose-설치}
...
## 2단계: Caddy (리버스 프록시 + 자동 HTTPS) {#2단계-caddy--리버스-프록시-plus-자동-https}


### 2.1 Docker 네트워크 생성 {#2-dot-1-docker-네트워크-생성}
...
### 2.2 docker-compose.yml {#2-dot-2-docker-compose-dot-yml}
...
### 2.3 Caddyfile {#2-dot-3-caddyfile}
...
## 3단계: Quartz (Obsidian → 디지털 가든) {#3단계-quartz--obsidian-디지털-가든}
...
### 3.2 서버에서 빌드하는 방식 {#3-dot-2-서버에서-빌드하는-방식}
...
### <span class="org-todo todo TODO">TODO</span> W님 볼트 구조 확인 후 Quartz 설정 커스터마이징 {#w님-볼트-구조-확인-후-quartz-설정-커스터마이징}


## 4단계: Remark42 (댓글 시스템) {#4단계-remark42--댓글-시스템}


### 4.1 docker-compose.yml {#4-dot-1-docker-compose-dot-yml}
...
### 4.2 Quartz에 Remark42 임베드 {#4-dot-2-quartz에-remark42-임베드}
...
## 5단계: Umami (방문자 분석) {#5단계-umami--방문자-분석}


### 5.1 docker-compose.yml {#5-dot-1-docker-compose-dot-yml}
...
## 6단계: OpenClaw (AI 에이전트) {#6단계-openclaw--ai-에이전트}
...
### 6.2 docker-compose.yml (기본형) {#6-dot-2-docker-compose-dot-yml--기본형}
...
### <span class="org-todo todo TODO">TODO</span> OpenClaw 정식 설치 — 정한님 SSH 접속 후 진행 {#openclaw-정식-설치-정한님-ssh-접속-후-진행}


## 운영 가이드 {#운영-가이드}


### 서비스 관리 명령어 {#서비스-관리-명령어}
...
### 디스크 관리 (프리티어 주의사항) {#디스크-관리--프리티어-주의사항}
...
### <span class="org-todo todo TODO">TODO</span> 도메인 보유 여부 확인 {#도메인-보유-여부-확인}


### <span class="org-todo todo TODO">TODO</span> Obsidian 볼트 구조/규모 확인 {#obsidian-볼트-구조-규모-확인}


### <span class="org-todo todo TODO">TODO</span> GitHub/Google OAuth 설정 가능 여부 {#github-google-oauth-설정-가능-여부}


### <span class="org-todo todo TODO">TODO</span> Quartz 테마/커스터마이징 취향 {#quartz-테마-커스터마이징-취향}


## 관련 노트 {#관련-노트}

-   [§entwurf 분신 에이전트 가이드]({{< relref "/botlog/20260324T153323.md" >}}) — 분신 운영 전체 맥락
