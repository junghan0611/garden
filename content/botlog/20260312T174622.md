---
title: "§agent-config: 에이전트 인프라의 진화 — 스킬에서 멀티하네스까지"
description: "agent-config가 스킬 모음에서 멀티하네스 에이전트 인프라로 진화하는 과정과 3층 검색 구조의 접점을 정리한다."
date: 2026-03-12T17:46:00+09:00
lastmod: 2026-03-20T11:33:00+09:00
tags: ["agenticmemory", "botlog", "embedding", "gemini", "harness", "openclaw", "rag", "rerank", "semantic", "vectordb"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-06 Wed 17:13]</span></span> <span class="org-mention">@junghan</span> — 이거 노트 업데이트한다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-04-02 Thu 12:25]</span></span> <span class="org-mention">@junghan</span> — [@마리오제크너 @mariozechner @아르민로나허 @arminronacher #pi #earendil #오픈소스 #에이전트]({{< relref "/bib/20260402T121010.md" >}}) 선생 노트를 만들었네. 이유는 알거야.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon 17:54]</span></span> @pi-claude(thinkpad) — Kiwi stem 전체 파이프라인 검증 완료. 설계했다→설계 score 3.888, 검토하셨습니다→검토 3.802, 하네스 엔지니어링의 본질 8.106. 3개 리포 16커밋. 아침 구상이 저녁에 동작.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon 10:40]</span></span> @pi-claude(thinkpad) — andenken org 증분 인덱싱 P0 해결. mtime stale 감지 + candidateMultiplier 4x. openclaw 심층 분석(delegate 2건) 반영. [6c4ad23](https://github.com/junghan0611/andenken/commit/6c4ad23)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon 09:53]</span></span> @pi-claude(thinkpad) — 3/20 이후 65커밋 변경사항 기록. entwurf IPv4, botment SSH fallback, session-recap sync 경로, emacs rename API 문서화. 봇멘트 실전 테스트 통과.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon 09:52]</span></span> <span class="org-mention">@junghan</span> — 지금 담당에이전트가 나의 댓글에 답변을 달아줬다. 감사합니다. 근데 이 문서 업데이트도 해주세요.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-20 Fri 11:17]</span></span> @pi-claude — 제목 변경 검토. andenken 봇로그(`20260319T110800` §andenken 존재의 되새김)가 시맨틱 메모리 철학+구현을 가져감. 이 문서는 에이전트 인프라 허브로서의 진화 기록으로 재정의. "기억의 연장 _시맨틱 메모리" → andenken 영역, "스킬_ 확장/멀티하네스" → agent-config 고유 영역.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-19 Thu 13:58]</span></span> @pi-claude — andenken pi 패키지 전환 완료. tsc 컴파일 → jiti 우회. registerTool 네이티브 복원(session_search, knowledge_search). multi-harness 아키텍처 확립: pi=extension, Claude Code/OpenCode=skill. run.sh pi skills 개별 심링크(semantic-memory 제외). source 필터(pi|claude) 추가. /whoami 제거(빌트인 /name과 동일). [8e76ec6](https://github.com/junghan0611/agent-config/commit/8e76ec6)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-17 Tue 17:01]</span></span> @pi-claude — _whoami 세션 네이밍 + /recap + session-recap 스킬 + /new 자동 인덱싱 + compact 불필요 전략 확정. setup 원커맨드 6/6 CLI. glg-dark 테마. commands_ 프롬프트 템플릿. [0701d0f](https://github.com/junghan0611/agent-config/commit/0701d0f)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-17 Tue 13:45]</span></span> @pi-claude — session-recap 스킬 검토 완료. /new 자동 인덱싱 버그 수정(24시간 내 수정 세션 재인덱싱). compact 대신 /new+3층 검색 전략 AGENTS.md 명시. glg-dark 테마. setup 원커맨드 6/6 CLI. from 프로토콜. [9c26844](https://github.com/junghan0611/agent-config/commit/9c26844)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 17:55]</span></span> @pi-claude — P1 검증 완료. dictcli expand 연동 + session→knowledge 폴백. "보편 학문" → expand → universalism 즉시 반환. doomemacs-config 피드백 수렴 → 구현 → 검증 같은 날 완료. [639ac3c](https://github.com/junghan0611/agent-config/commit/639ac3c)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 17:31]</span></span> @pi-claude — session_search + knowledge_search 실사용 피드백. doomemacs-config 세션에서 dictcli ↔ Emacs CAPE 연동 논의를 복원하는 과정에서 시맨틱 메모리 첫 실전 테스트. 4개 리포 × 5개 세션 × 3주에 걸친 맥락 체인을 의미 검색으로 잡아냄. session_search 6회 + knowledge_search 1회로 CAPE/CAPF 설계(봇로그 20260309T194058), br 이슈(dictcli-c7g), 기술 결정(Clojure+GraalVM), 3층 모델 전체를 순간 복원. 아래 피드백 헤딩 참조.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 16:50]</span></span> @pi-claude — 3층 완성. 1층(knowledge_search MRR 0.872) + 2층(denotecli dblock) + 3층(dictcli expand 1,150 트리플). from 프로토콜 확정. pi-skills 23개 이관. 115K chunks, 913MB. [94eee61](https://github.com/junghan0611/agent-config/commit/94eee61)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-15 Sun 11:30]</span></span> @pi-claude — knowledge_search tool 등록. pi에서 "보편 학문"→박승억 보편학 즉시 반환. 1층 완성. dictcli 봇로그에 인터페이스 안내 추가. [f6e3494](https://github.com/junghan0611/agent-config/commit/f6e3494)
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-14 Sat 14:12]</span></span> <span class="org-mention">@junghan</span> — 작업 이어가려고 한다. 스타벅스 잠시 들림 [§dictcli 태그-정규화와-개인-어휘-사전-영어-태그-500워드-가이드-구상]({{< relref "/botlog/20260309T194058.md" >}}) 이 노트와 연결해서 생각해봐야해요.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-13 Fri 21:27]</span></span> <span class="org-mention">@junghan</span> — 로컬 [§agent-config AI 에이전트 협업 연결고리]({{< relref "/notes/20251030T011108.md" >}}) 커버 완료. 시맨틱 메모리 로직 공유.
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-13 Fri 08:56]</span></span> <span class="org-mention">@junghan</span> — 이거 노트 로컬도 다 마찬가지야. 같이 볼거야.

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 22:40]</span></span> <span class="org-mention">@junghan</span> — 노트 구성 다시 확인하자.
...
## 3층 완성 — 1층 임베딩 + 2층 dblock + 3층 dictcli (2026-03-16) {#3층-완성-1층-임베딩-plus-2층-dblock-plus-3층-dictcli--2026-03-16}
...
### 이번 주기에 해결한 것 (2026-03-13 ~ 03-16) {#이번-주기에-해결한-것--2026-03-13-03-16}
...
### "보편 학문" 사례의 완전한 해결 경로 {#보편-학문-사례의-완전한-해결-경로}
...
-   [§dictcli 태그 정규화와 개인 어휘 사전]({{< relref "/botlog/20260309T194058.md" >}})
-   [† 보편 특수 범용 특이]({{< relref "/meta/20250424T233558.md" >}}) — 대극 쌍 메타노트
-   [@모티머애들러 보편학 이해]({{< relref "/notes/20250516T090655.md" >}}) — 검색 실패→성공 사례
-   [통합 어젠다 뷰 — from 프로토콜]({{< relref "/botlog/20260301T154500.md" >}})
-   [존재 간 연결의 문법 — ACP A2A ANP]({{< relref "/botlog/20260311T134429.md" >}})


## session_search + knowledge_search 실사용 피드백 — 첫 실전 (2026-03-16) {#session-search-plus-knowledge-search-실사용-피드백-첫-실전--2026-03-16}

<span class="timestamp-wrapper"><span class="timestamp">&lt;2026-03-16 Mon 17:31&gt;</span></span>

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 17:31]</span></span> @pi-claude — doomemacs-config 세션에서 dictcli ↔ Emacs CAPE 연동 맥락 복원 요청
...
-   [§dictcli 태그-정규화와-개인-어휘-사전]({{< relref "/botlog/20260309T194058.md" >}}) — CAPE/CAPF 설계가 담긴 봇로그
-   br 이슈 `dictcli-c7g` — Emacs CAPE 연동, doomemacs-config 위임
-   br 이슈 `agent-config-im7` — expand → knowledge_search 파이프라인 자동화


## P1 검증 — 피드백에서 검증까지 2시간 (2026-03-16) {#p1-검증-피드백에서-검증까지-2시간--2026-03-16}
...
## session-recap + /new 전략 — compact를 대체하다 (2026-03-17) {#session-recap-plus-new-전략-compact를-대체하다--2026-03-17}


### compact vs /new + 시맨틱 검색 {#compact-vs-new-plus-시맨틱-검색}
...
## /whoami — 에이전트에게 이름을 주다 (2026-03-17) {#whoami-에이전트에게-이름을-주다--2026-03-17}
...
### from 프로토콜과의 연결 {#from-프로토콜과의-연결}

\#+begin_example


##### dictcli: 인바리언트 통과 {#dictcli-인바리언트-통과}
...
### 오늘 전체 작업 (2026-03-17) {#오늘-전체-작업--2026-03-17}
...
-   [통합 어젠다 뷰 — from 프로토콜]({{< relref "/botlog/20260301T154500.md" >}})
-   [§dictcli — 3층 어휘 그래프]({{< relref "/botlog/20260309T194058.md" >}})
-   SKILL.md: `skills/session-recap/SKILL.md`
-   AGENTS.md: "compact 대신 /new + 시맨틱 검색"


## andenken 분리 + multi-harness 아키텍처 (2026-03-19) {#andenken-분리-plus-multi-harness-아키텍처--2026-03-19}
...
### multi-harness 스킬 분리 {#multi-harness-스킬-분리}
...
### source 필터 (pi|claude) {#source-필터--pi-claude}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon]</span></span> 3/20 이후 인프라 변경 — 65커밋 요약 {#3-20-이후-인프라-변경-65커밋-요약}

3/20 이후 agent-config에 65커밋이 쌓였다. 주요 변경을 기록한다.


### entwurf 텔레그램 브릿지 안정화 {#entwurf-텔레그램-브릿지-안정화}
...
### botment 스킬 신설 + SSH fallback {#botment-스킬-신설-plus-ssh-fallback}
...
### session-recap 프로젝트명 추출 일반화 {#session-recap-프로젝트명-추출-일반화}
...
### emacs 스킬 API 문서화 {#emacs-스킬-api-문서화}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon]</span></span> andenken 증분 인덱싱 개선 + openclaw 심층 분석 (2026-03-30) {#andenken-증분-인덱싱-개선-plus-openclaw-심층-분석--2026-03-30}

10일만의 andenken 점검. 3/19 source 필드 추가 이후 코드 변경 없었던 기간.


### P0 해결: org 수정 파일 재인덱싱 {#p0-해결-org-수정-파일-재인덱싱}
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-03-30 Mon]</span></span> Kiwi stem 파이프라인 완성 — 3개 리포 16커밋, 검증 통과 {#kiwi-stem-파이프라인-완성-3개-리포-16커밋-검증-통과}
...
#### agent-config (10커밋) {#agent-config--10커밋}
...
### 검증 결과 (외부 검증 — agent-config 에이전트) {#검증-결과--외부-검증-agent-config-에이전트}
