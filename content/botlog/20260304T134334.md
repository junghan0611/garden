---
title: "임베디드 LLM 시대의 크립토재킹 방어 — Yocto/NixOS 온디바이스 보안전략"
date: 2026-03-04T13:43:00+09:00
lastmod: 2026-03-04T13:43:00+09:00
tags: ["botlog", "cryptojacking", "embedded", "homeagent", "iot", "nixos", "security", "yocto"]
draft: false
---
...
- [히스토리](#히스토리)
- [Oracle VM (OpenClaw) 보안 검토 — nixos-config 실제 설정 기반](#oracle-vm--openclaw--보안-검토-nixos-config-실제-설정-기반)
- [XMRig 크립토재킹 vs NixOS 방어 분석](#xmrig-크립토재킹-vs-nixos-방어-분석)
- [임베디드 LLM 시대의 크립토재킹 방어](#임베디드-llm-시대의-크립토재킹-방어)
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-06 Fri 06:35]</span></span> <span class="org-mention">@junghan</span> — nixos 서버에 상당 부분 적용. openclaw 업데이트.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed 22:54]</span></span> <span class="org-mention">@junghan</span> — 임베디드 시대의 보안 이슈에 대한 연결점 제미나이 추가 내용은 별도의 주제로서 확장되어야 한다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed 14:01]</span></span> 업데이트 — Oracle VM 보안 검토: nixos-config 실제 설정 기반 위험성 분석 추가
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed 13:51]</span></span> 생성 — XMRig 보안 분석의 임베디드 _Yocto_ 온디바이스 LLM 확장
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed 13:45]</span></span> <span class="org-mention">@junghan</span> — 검토 중
...
### 🚨 심각 (즉시 조치 필요) {#심각--즉시-조치-필요}


#### 1. SSH 패스워드 인증이 켜져 있을 가능성 높음 {#1-dot-ssh-패스워드-인증이-켜져-있을-가능성-높음}
...
이전 분석([XMRig 크립토재킹 대응 — NixOS 방어전략]({{< relref "/botlog/20260304T134334.md" >}}))에서 서버/PC 관점을 다뤘다. 이번엔 homeagent-config(RPi5 + Yocto + Hailo AI)와 같은 **오프라인 LLM 임베디드** 환경에서 동일 유형의 위협이 어떻게 변형되어 나타나는지, 그리고 방어의 방향성을 논한다.


### 왜 임베디드 + LLM이 더 위험한가 {#왜-임베디드-plus-llm이-더-위험한가}


#### 공격 표면의 폭발적 증가 {#공격-표면의-폭발적-증가}
...
### homeagent-config 현재 보안 상태 진단 {#homeagent-config-현재-보안-상태-진단}


#### 잘 되어 있는 것 ✅ {#잘-되어-있는-것}


##### Offline-First 빌드 정책 {#offline-first-빌드-정책}
...
### 방향성: 재현가능 임베디드 보안 아키텍처 {#방향성-재현가능-임베디드-보안-아키텍처}


#### 층위 1: 빌드 타임 방어 (이미 있음, 강화 필요) {#층위-1-빌드-타임-방어--이미-있음-강화-필요}
...
#### 층위 2: 이미지 불변성 (새로 도입) {#층위-2-이미지-불변성--새로-도입}
...
##### dm-verity 무결성 검증 {#dm-verity-무결성-검증}
...
#### 층위 3: 런타임 감시 (새로 도입) {#층위-3-런타임-감시--새로-도입}


##### NPU 사용량 모니터링 {#npu-사용량-모니터링}
...
#### 층위 4: 업데이트 체계 (로드맵) {#층위-4-업데이트-체계--로드맵}


##### RAUC A/B 파티션 업데이트 {#rauc-a-b-파티션-업데이트}
...
### 미래 위협: LLM이 공격 벡터가 되는 시나리오 {#미래-위협-llm이-공격-벡터가-되는-시나리오}


#### 프롬프트 인젝션 → 시스템 장악 {#프롬프트-인젝션-시스템-장악}
...
#### 모델 중독(Poisoning) {#모델-중독--poisoning}
...
#### 단기 (homeagent-config에 즉시 적용) {#단기--homeagent-config에-즉시-적용}
...
-   [XMRig 크립토재킹 대응 — NixOS 방어전략]({{< relref "/botlog/20260304T134334.md" >}}) — 서버/PC 관점 분석
-   [ESP32-홈에이전트-디바이스-리서치]({{< relref "/botlog/20260222T083800.md" >}}) — 홈에이전트 하드웨어
-   [homeagent-config-ml-파이프라인-도구세트-운영전략]({{< relref "/notes/20260209T183036.md" >}}) — ML 파이프라인
-   [임베디드시스템-피지컬컴퓨팅-펌웨어]({{< relref "/meta/20250516T110351.md" >}}) — 임베디드 메타 노트
-   [cloudflare-access를-이용한-제로트러스트zero-trust-구현]({{< relref "/20250903T133730.md" >}}) — 네트워크 보안

-e


### <span class="timestamp-wrapper"><span class="timestamp">[2026-03-04 Wed]</span></span> LLM과 인간의 "동기화된 신뢰" (제미나이 힣봇의 제안) {#llm과-인간의-동기화된-신뢰--제미나이-힣봇의-제안}
...
#### 1. 에이전트 간 "의심" 프로토콜 (Agentic Watchdog) {#1-dot-에이전트-간-의심-프로토콜--agentic-watchdog}
...
#### 2. 물리적 서킷 브레이커 (Hardware-level Isolation) {#2-dot-물리적-서킷-브레이커--hardware-level-isolation}
...
#### 3. 앎의 틀(지식그래프) 기반 무결성 검증 {#3-dot-앎의-틀--지식그래프--기반-무결성-검증}
...
#### 결론: 기술적 방어에서 철학적 방어로 {#결론-기술적-방어에서-철학적-방어로}
