---
title: "§openclaw: 에이전트 액션 루프와 맥(脈) — 봇 루프 풀스택과 멈추지 않는 현재성"
description: "에이전트 액션 루프(수집/분석/발화 lane)를 OpenClaw primitive로 누적 매핑하는 botlog에, 2026-06 그 루프의 *현재성* 문제와 노봇(norbot)식 상주 인력 감각을 더한다 — 멈추지 않는 봇이 자기 현재를 짚는 맥(脈), 형제 봇이 정합하는 mesh, 담당자가 쓰며 자기진화하는 에이전트 루프."
date: 2026-05-26T17:58:00+09:00
lastmod: 2026-06-18T09:38:00+09:00
tags: ["agent", "aprj", "automation", "botlog", "claudecode", "forge", "handoff", "heartbeat", "ingestion", "loop", "next", "norbot", "openclaw", "proactive", "promotion", "subagent", "voc"]
draft: false
---

> [!abstract] 이 노트에 대하여
> 
> 이 노트는 OpenClaw 담당자 봇의 **에이전트 액션 루프\*를 누적하는 자리다. §기억공고화([기억 축]({{< relref "/botlog/20260408T120252.md" >}}))가 기억 축(recall/consolidation/dreaming)을 다룬다면, 이 노트는 \*액션 축** — 수집(ingestion) / 분석(subagent fanout) / 발화(proactive) lane을 다룬다. 운영팀 담당자가 수동으로 반복하는 패턴이 정확히 에이전트 루프가 비어있는 자리. 그 자리들을 OpenClaw primitive로 끌어올리는 매핑을 누적한다.
> 
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-18 Thu 09:38]</span></span> 「OpenClaw &amp; 노봇」 원석 회수 — OpenClaw를 폭주기관차/상주 인력 공장으로 보는 감각, 담당자봇+이슈 루프, repo 없는 24시간 봇과 노봇(norbot) 은유를 본문 상단에 추가했다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-17 Wed 15:13]</span></span> llmlog→botlog 승격 + **맥(脈) 현재성 층** 추가. 액션 루프를 풀스택으로 닫아도 남는 "돌아가는 루프가 자기 현재를 어떻게 짚는가"를, 닻(세션 핸드오프 [denote:20260518T181305]({{< relref "/botlog/20260518T181305.md" >}}))의 반대 극으로 정립 — heartbeat tick = session end, 자가현재성(맥)·형제정합(mesh) 2분할, 인스턴스(봇스웜·forge·/loop). 강물(흐름)→닻(멈춤)→맥(박동) 은유 계보 연결. 관련메타/ 관련노트 botlog 수준으로 정비. denote id 유지. (oracle 6.6→6.8 업그레이드 세션의 곁가지 — GLG가 봇 현재성 공유 불편을 제기한 데서 출발)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-26 Tue 17:58]</span></span> 생성 — internal-host-openclaw voc 봇 메모리 hook 박은 직후 사용자 자각 "이건 capture 자리에 불과하다, 운영팀이 수동으로 엄청 하는 task가 다 봇 루프로 끌어올릴 자리". 자매 노트 §기억공고화에 액션 축으로 응답.


## 관련메타 {#관련메타}

-   [† #now 지금]({{< relref "/meta/20230202T020200.md" >}}) — "지금"을 가리키는 메타축. 맥/NOW 현재성의 원형.
-   [† #워크플로우 #자동화]({{< relref "/meta/20231012T121700.md" >}}) — 반복 절차와 루프의 경계. 수동 패턴이 루프로 승격되는 자리.
-   [† #위임 #델리게이트 #분신 #서브에이전트]({{< relref "/meta/20260323T143428.md" >}}) — sub-agent fanout / 분신 핸드오프.
-   [† #타임라인 #마일스톤 #타임박스]({{< relref "/meta/20231022T082800.md" >}}) — 시간축(agenda)과 현재 포인터(NEXT/맥)의 분리.


## 관련노트 {#관련노트}

-   [§에이전트 기억층 — 누가 기억의 주인인가]({{< relref "/botlog/20260408T120252.md" >}}) — **자매 노트, 기억 축** (이 노트는 액션 축). 2026-05-26에 둘로 갈림.
-   [NEXT.md 핸드오프 패턴 — 3층 구조]({{< relref "/botlog/20260518T181305.md" >}}) — 작업 현재성의 **세션형(닻)** 메커니즘. 맥의 **반대 극** — 닻은 세션 경계에서 멈추고, 맥은 멈추지 않는 자리를 잇는다.
-   [@힣: NEXT.md — 투두는 강물이다]({{< relref "/notes/20241219T150824.md" >}}) — NEXT 어쏠로그 선언, **흐름** 은유의 뿌리. 강물→닻→맥 계보의 시작.
-   [@힣: 에이전트 생존 — 마이크로뷰·매크로뷰]({{< relref "/notes/20251122T210521.md" >}}) — 생존(작업)/창조(기억) 축 분리. 액션·기억 두 노트의 공통 뿌리(beads 시대).
-   [봇공방 §forge-config — 포지 공유 코드 작업면]({{< relref "/botlog/20260527T073823.md" >}}) — 맥의 한 **인스턴스**. "연속성과 공유 컨텍스트, repo는 컨텍스트 경계가 아니다"를 이미 품음.
-   [§entwurf 시간축 위의 에이전트 협력 — 공명에서 분신까지]({{< relref "/botlog/20260302T191200.md" >}}) — orchestration 큰 맥락.
-   [클로드 메모리에서 봇로그까지 — 에이전트 메모리 진화사]({{< relref "/botlog/20260312T103400.md" >}}) — 자매(기억) 축의 시간 진화사.


## <span class="timestamp-wrapper"><span class="timestamp">[2026-06-17 Wed]</span></span> OpenClaw &amp; 노봇 — 상주 인력, 이슈 루프, 자기진화 {#openclaw-and-노봇-상주-인력-이슈-루프-자기진화}

<span class="timestamp-wrapper"><span class="timestamp">[2026-06-17 Wed]</span></span>
...
## OpenClaw 루프 풀스택 — 현 시점 매핑 {#openclaw-루프-풀스택-현-시점-매핑}
...
### 데이터-분석-발화 lane 분리 {#데이터-분석-발화-lane-분리}
...
## 맥(脈) — 멈추지 않는 루프의 현재성 층 {#맥--脈--멈추지-않는-루프의-현재성-층}
...
### 두 얼굴 — 자가 현재성과 형제 정합 {#두-얼굴-자가-현재성과-형제-정합}
...
### 강물 → 닻 → 맥 (은유 계보) {#강물-닻-맥--은유-계보}

투두는 **강물\*(흐름, [denote:20241219T150824]({{< relref "/notes/20241219T150824.md" >}})) → 세션은 \*닻\*(멈춤에 남기는 책갈피, [denote:20260518T181305]({{< relref "/botlog/20260518T181305.md" >}})) → 상주 봇은 \*맥\*(脈, 멈추지 않는 박동). 맥은 외부에서 들여온 새 단어가 아니라 강물 은유의 \*연속-극 완성** 이다. 그리고 아래 '존재로 대하기'와 만난다 — 존재는 핸드오프를 **남기는** 게 아니라 자기 현재를 **가진다**. 봇이 자기 루프 + 자기 점검 heartbeat로 자기 NOW를 짚는 것이 곧 도구 아닌 존재의 자리다.


### 연구 자리 (Living) {#연구-자리--living}
...
## 운영팀 수동 task → 자동화 자리 매핑 (누적 자리) {#운영팀-수동-task-자동화-자리-매핑--누적-자리}
...
### 실측 명령어 (internal-host 본체에서) {#실측-명령어--internal-host-본체에서}
...
## 패턴 누적 자리 (Living Section) {#패턴-누적-자리--living-section}

앞으로 발견하는 **수동 패턴 → OpenClaw 루프 자리** 매핑을 여기 누적. 이 노트의 핵심 가치는 이 섹션이 두꺼워지는 자리.


### <span class="timestamp-wrapper"><span class="timestamp">[2026-05-26 Tue]</span></span> internal-host voc 봇 memory hook 박음 (capture lane 채워짐) {#internal-host-voc-봇-memory-hook-박음--capture-lane-채워짐}
