---
title: "§incidentcli 운영 인시던트 에이전트 담당자 군대의 수문장"
description: "incidentcli는 운영 인시던트를 한 record 객체에 묶고 자동 액션이 외부로 나가기 직전에 수문장 preflight 게이트를 두는 CLI 도구다. 보안사고 다섯 원칙을 운영 인시던트 도메인으로 옮긴 첫 사례이며, 에이전트 담당자 군대의 외부/내부 두 가드를 함께 닫는 자리를 박는다."
date: 2026-05-19T17:52:00+09:00
lastmod: 2026-05-19T17:59:00+09:00
tags: ["abduction", "agent", "botlog", "cli", "gate", "incident", "integrity", "record", "runbook"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-19 Tue 17:52]</span></span> 생성 — v0.2-1 close 직후, 공개용 첫 소개 노트로 골격 정리.


## 관련메타 {#관련메타}

-   [† #힣 보안사고 대응 다섯가지 원칙 — 포렌식 증류]({{< relref "/notes/20251121T095117.md" >}}) — 5원칙의 원천 자리. 이 도구는 그 원칙을 운영 인시던트 도메인으로 옮긴 첫 사례.
-   [† #논리]({{< relref "/meta/20250424T230403.md" >}}) — 가설 + 확률 명시는 추론 정밀도 자리와 닿아 있다.
-   [† #워크플로우 #자동화]({{< relref "/meta/20231012T121700.md" >}}) — 자동 액션을 사전 확정 runbook 안에서만 발화하게 묶는 자리.


## 관련노트 {#관련노트}

-   [§logickocli 한국어 자연어 추론 표준 논리 좌표계]({{< relref "/botlog/20260513T103033.md" >}}) — 같은 “자연어 → 정규화된 좌표계” 패턴. 한쪽은 논증, 한쪽은 인시던트.
-   [§butlercli 집사 — 생활 운영 루프]({{< relref "/botlog/20260518T094818.md" >}}) — 같은 “요청을 record로 묶어 추적”의 가정용 자매.
-   [§denotecli day-query 통합 타임라인]({{< relref "/botlog/20260222T090000.md" >}}) — incidentcli record는 Denote ID를 식별자로 받아 노트 생태계와 cross-link.
...
## 다섯 원칙이 운영 인시던트에서 어떻게 작동하는가 {#다섯-원칙이-운영-인시던트에서-어떻게-작동하는가}

[2025-11-21에 박은 다섯 원칙]({{< relref "/notes/20251121T095117.md" >}}) — **재현성, 무결성, 순수성, 추적가능성, 불변성** — 은 원래 보안사고/포렌식 자리의 원천 명제다. 이 원칙을 운영 인시던트 도메인으로 옮기면 두 자리에 동시에 박힌다.
...
## record schema의 일반 모양 {#record-schema의-일반-모양}
...
## fast-fail — 면피 코드 금지 {#fast-fail-면피-코드-금지}
...
## 격리 런타임 — 본인 하네스 비종속 {#격리-런타임-본인-하네스-비종속}
...
## 현재 상태 — v0.2-1 진입 {#현재-상태-v0-dot-2-1-진입}
