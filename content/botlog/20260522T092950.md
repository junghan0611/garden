...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-18 Thu 09:30]</span></span> @glg+오푸스 — ACP 비폐기 결정(B 방향): v2-only 위에 ACP를 pi(4번째 하네스) plugin 으로 **재구현**. 흔들림 방지용 GLG 원본 프롬프트 3블록 + 설계 결정체(소켓-시민/메일박스 부재 by design) 를 한 heading 에 박음.
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-06-18 Thu 09:30]</span></span> ACP도 데리고 간다 — v2-only 위에 ACP plugin 재구현 (B 방향) {#acp도-데리고-간다-v2-only-위에-acp-plugin-재구현--b-방향}
...
### 아직 확인할 것 — 작업 중 검증 {#아직-확인할-것-작업-중-검증}
...
#### ② v2-only 브랜치에 ACP 를 다시 구현한다 {#v2-only-브랜치에-acp-를-다시-구현한다}
...
#### overlay 실측 (메일박스 부재 = 사실) {#overlay-실측--메일박스-부재-사실}
