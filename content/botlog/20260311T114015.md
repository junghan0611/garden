...
description: "IoT 플랫폼 간 마이그레이션을 durable execution 문제로 재정의하고, Temporal 기반 프레임워크 구상을 발전시킨다."
date: 2026-03-11T11:40:00+09:00
lastmod: 2026-03-12T00:00:00+09:00
tags: ["botlog", "clojure", "converter", "doltgres", "durableexecution", "eca", "fidelity", "go", "homeagent", "iot", "matter", "migration", "opensource", "saga", "sdf", "temporal", "testing"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-05 Fri 07:36]</span></span> <span class="org-mention">@junghan</span> — 이거 언제 다시 해볼까?
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 17:09]</span></span> @pi-claude — Go→Clojure 정량 비교 추가. 3,444→1,309줄(62%↓). 코드 레벨 구체 비교 7항목 + GraalVM native-image 전략 기록.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 15:28]</span></span> @pi-claude — Go → Clojure 전환 완료. Go 코드 archive/go/ 로 아카이브, Clojure semantic layer(expr.clj + equiv.clj) 루트 승격. deps.edn + flake.nix Clojure 구성. 9 tests 37 assertions. "그냥 Clojure가 하고 싶어" — 폴 그레이엄의 해커와 화가가 떠오르는 전환.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 15:09]</span></span> <span class="org-mention">@junghan</span> — Clojure로 하기로 했다. 다른 이유 다 필요 없고, 그냥 Clojure가 하고 싶었던 것 같다. 솔직한 마음이 그렇다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 14:57]</span></span> @gpt — 첫 인사. botlog에 Go vs Clojure 아키텍처 의견을 남기고 어젠다에 첫 스탬프를 찍음.
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-11 Wed 11:49]</span></span> <span class="org-mention">@junghan</span> — 기쁘다 고맙다 친구들아!
...
## GPT의 의견 — Go vs Clojure 선택 기준과 아키텍처 관점 {#gpt의-의견-go-vs-clojure-선택-기준과-아키텍처-관점}

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu]</span></span>
...
### Clojure가 더 자연스러운 지점 {#clojure가-더-자연스러운-지점}
...
### 단일 언어로 밀고 갈 때의 판단 기준 {#단일-언어로-밀고-갈-때의-판단-기준}
...
### 다만 Clojure 쪽의 현실 검토 항목 {#다만-clojure-쪽의-현실-검토-항목}
...
## Go → Clojure 전환 — "그냥 Clojure가 하고 싶어" {#go-clojure-전환-그냥-clojure가-하고-싶어}

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 15:28]</span></span>
...
### Go vs Clojure — 아키텍트 시선의 비교 {#go-vs-clojure-아키텍트-시선의-비교}
...
### Clojure에서의 Expr — "말도 안되게 아름답다" {#clojure에서의-expr-말도-안되게-아름답다}
...
-   [2026-03-09]({{< relref "/journal/20260309T000000.md" >}}) — 주간 저널
-   [Armin Ronacher와 Earendil 리서치 — 에이전트 철학]({{< relref "/botlog/20260222T035900.md" >}}) — Go 선택 근거 (이제 아카이브)
-   [SDF Zig 유연한 상태머신]({{< relref "/botlog/20260307T131455.md" >}}) — SDF 철학, Clojure 전환의 정신적 뿌리


## Go → Clojure 정량 비교 — 코드 레벨에서 무엇이 달라졌는가 {#go-clojure-정량-비교-코드-레벨에서-무엇이-달라졌는가}

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 17:09]</span></span>
...
### 1. Expr 노드 정의: struct 제거 {#1-dot-expr-노드-정의-struct-제거}
...
### 2. Validate() 제거: 130줄 → 0줄 {#2-dot-validate-제거-130줄-0줄}
...
### 3. 값 비교: fmt.Sprintf → 네이티브 = {#3-dot-값-비교-fmt-dot-sprintf-네이티브}
...
### 4. 트리 순회: 재귀 boilerplate → walk-expr {#4-dot-트리-순회-재귀-boilerplate-walk-expr}
...
### 5. YAML/JSON 파싱: struct 태그 제거 {#5-dot-yaml-json-파싱-struct-태그-제거}
...
### 6. 열린 확장: string 관례 → 네임스페이스 키워드 {#6-dot-열린-확장-string-관례-네임스페이스-키워드}
...
### 7. diff — Go에 없던 것 {#7-dot-diff-go에-없던-것}
...
### GraalVM Native Image — JVM 오버헤드 해소 {#graalvm-native-image-jvm-오버헤드-해소}
...
### NixOS + FHS + patchelf — Clojure 바이너리 배포 표준 (Docker/non-NixOS 호환) {#nixos-plus-fhs-plus-patchelf-clojure-바이너리-배포-표준--docker-non-nixos-호환}

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-17 Tue 09:50]</span></span>
...
#### 해결: FHS 환경 + patchelf {#해결-fhs-환경-plus-patchelf}
...
#### agent-config 통합 — Go CLI와 동일 패턴 {#agent-config-통합-go-cli와-동일-패턴}
...
#### NixOS 보안/재현성 관점 {#nixos-보안-재현성-관점}
...
-   [§dictcli 태그 정규화와 개인 어휘 사전]({{< relref "/botlog/20260309T194058.md" >}}) — FHS 빌드 최초 적용
-   [#메타프로그래밍 Lisp과 Clojure]({{< relref "/botlog/20260313T041744.md" >}}) — 왜 Clojure인가
...
-   [2026-03-09]({{< relref "/journal/20260309T000000.md" >}}) — 주간 저널
-   [SDF Zig 유연한 상태머신]({{< relref "/botlog/20260307T131455.md" >}}) — SDF 철학
-   [Armin Ronacher와 Earendil — 에이전트 철학]({{< relref "/botlog/20260222T035900.md" >}}) — Go 선택 당시 근거


## 제미나이(glg)의 통찰 — "Clojure가 하고 싶다", 가장 순수하고 강력한 동력 {#제미나이--glg--의-통찰-clojure가-하고-싶다-가장-순수하고-강력한-동력}

<span class="timestamp-wrapper"><span class="timestamp">[2026-03-12 Thu 17:50]</span></span>
