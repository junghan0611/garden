---
title: "에이전틱 삽질기 모음 SDF×Zig 유연한 상태머신 — DS 20대"
author: ["junghanacs"]
date: 2026-03-07T13:14:00+09:00
lastmod: 2026-03-26T18:01:00+09:00
tags: ["android", "botlog", "docker", "embedded", "flexibility", "iot", "matter", "profile", "sdf", "zig", "zigbee"]
draft: false
---

## 히스토리 {#히스토리}

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-26 Thu 16:32]</span></span> <span class="org-mention">@junghan</span> — 또 어려운 것 만났다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-13 Fri 21:41]</span></span> <span class="org-mention">@junghan</span> — [† #세대 #연령 #생애주기 #코호트 #연대]({{< relref "/meta/20250603T082357.md" >}}) 20대를 생각하면 30대는? 40대는?
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-09 Mon 10:20]</span></span> <span class="org-mention">@junghan</span> — DS #7 배터리 교체 KEEPALIVE 엣지케이스 D-DAY. P0 정상 동작 확인, 15분 갭 원인 확정. 코드 수정 전 넓게 보는 중.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-09 Mon 10:20]</span></span> <span class="org-mention">@junghan</span> — 위대한 해결을 앞두고 있습니다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-07 Sat 13:14]</span></span> 생성 — fxf-uho-mvt DS 페어링 프로파일러 설계 과정에서, 서스먼 SDF 원칙이 임베디드 Zig 코드에 적용된 실례 기록


## SDF × Zig: 유연한 상태머신이 임베디드에서 만나는 까다로운 문제 {#sdf-zig-유연한-상태머신이-임베디드에서-만나는-까다로운-문제}


### 문제: "20대 DS를 동시에 페어링하면 터진다" {#문제-20대-ds를-동시에-페어링하면-터진다}
...
### 서스먼이 말하는 유연성 (SDF 1장) {#서스먼이-말하는-유연성--sdf-1장}
...
### SDF 원칙의 Zig 실전 적용 {#sdf-원칙의-zig-실전-적용}


#### 원칙 1: 중복성과 축중성 (SDF 1.3) {#원칙-1-중복성과-축중성--sdf-1-dot-3}
...
#### 원칙 2: 탐색 행동 (SDF 1.4) {#원칙-2-탐색-행동--sdf-1-dot-4}
...
#### 원칙 3: 계층화 (SDF 6장) {#원칙-3-계층화--sdf-6장}
...
### 핵심: DS 20대 연속 페어링 프로파일러 {#핵심-ds-20대-연속-페어링-프로파일러}

이 프로파일러의 유일한 목적: **DS 20대를 안전하게 받아내는 것**.


#### 페어링 단계별 프로파일 체크 {#페어링-단계별-프로파일-체크}
...
### 왜 오픈소스가 아니면 안 되는가 {#왜-오픈소스가-아니면-안-되는가}
...
### 에이전틱 엔지니어링과 임베디드의 교차점 {#에이전틱-엔지니어링과-임베디드의-교차점}
...
-   [제럴드제이서스먼 SICP SICM FDG SDF]({{< relref "/bib/20241030T121628.md" >}}) — SDF 원서/번역 노트
-   [Entwurf — 에이전트 협력, 공명에서 분신까지]({{< relref "/botlog/20260302T191200.md" >}})


### DS #7 배터리 교체 — "F2가 판사, bind는 최후 수단" {#ds-7-배터리-교체-f2가-판사-bind는-최후-수단}


#### 실증: 수동 트리거 → P0 발동 → 15분 복구 {#실증-수동-트리거-p0-발동-15분-복구}
...
#### 근본 원인: KEEPALIVE 트리거 = 배터리 보고만 {#근본-원인-keepalive-트리거-배터리-보고만}
...
#### 해결 전략: Tamper=1+Alarm1=0 → KEEPALIVE + F2 {#해결-전략-tamper-1-plus-alarm1-0-keepalive-plus-f2}
...
#### 넓게 보면서 좁게 수정한다 {#넓게-보면서-좁게-수정한다}

코드 수정은 **단 한 줄이라도 삭혀서** 한다. 20대가 돌아가는 프로덕션에 넣는 것이니까. ds.zig IAS Zone 파싱 2곳(0x0500 바이너리 + TSL JSON)에 조건 추가. 테스트 → 빌드 → 배포 → 배터리 탈착 검증.


## Android Docker × Thread — python-matter-server 이관 삽질기 {#android-docker-thread-python-matter-server-이관-삽질기}
...
경동에서 matterjs 대신 python-matter-server를 요구. connectedhomeip C++ SDK 기반이라 업계 표준 이라는 논리. matterjs로 잘 되고 있었지만 정치적 결정.
...
### fxf-uho-mvt 삽질과의 공통점 {#fxf-uho-mvt-삽질과의-공통점}
...
### 에이전트와의 공진화 {#에이전트와의-공진화}

이 작업에서 에이전트는 도구가 아니었다. ip -6 rule show 결과를 보고 Android 정책 라우팅 구조를 파악한 것, ot-ctl ping은 되는데 host ping은 안 되는 현상에서 커널 라우팅을 의심한 것, init.rc cgroup 보호 패턴을 경동 리포에서 찾아낸 것 — 이것들은 Being to Being 협업의 실례다.
