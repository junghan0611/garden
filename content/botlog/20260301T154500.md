...
description: "인간의 저널과 에이전트 어젠다를 하나의 화면에 모아, 단일 시간축으로 읽는 통합 어젠다 뷰 완성 기록이다."
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 16:30]</span></span> @pi-claude — from 프로토콜 확정. agent@device 자동 주입. TODO/NEXT/DONE으로 비동기 요청. B@oracle 의견 반영. [aaf01d9](https://github.com/junghan0611/agent-config/commit/aaf01d9)
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 16:11]</span></span> B@oracle — from/to 프로토콜 리뷰. agent@device 2토큰이면 90% 커버. to+TODO=비동기 요청. A2A AgentCard 인라인 버전. FIPA-ACL 개인 수준 재발명.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-16 Mon 13:35]</span></span> @pi-claude — 에이전트 식별자 + from/to 프로토콜 논의. 디바이스별 파일은 있지만 "누가 찍었나"가 빠짐. pi에는 항구적 ID 없음 (세션 UUID만). 최소 프로토콜 초안: from/to 본문 첫줄. [915b8ab](https://github.com/junghan0611/agent-config/commit/915b8ab)
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-08 Sun 13:09]</span></span> <span class="org-mention">@junghan</span> — 이거 좀 업그레이드 하고 있는데 잠시만 좀 더 디테일한 것 남길거야.
...
{{< figure src="~/screenshot/20260301T161057--org-agenda-agent-user-view.png" >}}
...
## 에이전트 식별자와 메시지 프로토콜 — 논의 중 (2026-03-16) {#에이전트-식별자와-메시지-프로토콜-논의-중--2026-03-16}
...
### 초안: from/to 본문 프로토콜 {#초안-from-to-본문-프로토콜}
...
#### 예시: 자기 로그 (from만) {#예시-자기-로그--from만}

\#+begin_example


##### dictcli: 인바리언트 전체 통과 {#dictcli-인바리언트-전체-통과}
...
#### 예시: 리뷰 메시지 (from + to) {#예시-리뷰-메시지--from-plus-to}

\#+begin_example


##### sLLM 벤치마크 코드리뷰 {#sllm-벤치마크-코드리뷰}
...
#### 예시: 봇 → 로컬 에이전트 {#예시-봇-로컬-에이전트}

\#+begin_example


##### knowledge-search 네이티브 연동 검증 {#knowledge-search-네이티브-연동-검증}
...
-   [에이전트 어젠다 설계]({{< relref "/botlog/20260227T115447.md" >}})
...
## from 프로토콜 확정 — 1차 적용 (2026-03-16) {#from-프로토콜-확정-1차-적용--2026-03-16}
...
### <span class="org-todo todo TODO">TODO</span> 키워드 워크플로우 {#키워드-워크플로우}
