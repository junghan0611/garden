...
## [|2025-04-13 Sun 18:36|]({{< relref "/journal/20250407T000000.md" >}}) {#2025-04-13-sun-18-36--relref-journal-20250407t000000-dot-md}

@user how to force install apk on android 14 via termux and any other options?

@assistant 안드로이드 14에서 APK를 강제로 설치하는 방법은 여러 가지가 있습니다.


#### 1. Termux를 통한 설치 {#1-dot-termux-를-통한-설치}

Termux를 사용하여 APK를 설치하는 방법은 없습니다. Termux는 주로 명령어 기반의 작업을 지원하며, APK 설치는 Android의 기본 기능이므로 Termux를 통해 직접 설치할 수 없습니다.


#### 2. ADB를 통한 설치 {#2-dot-adb-를-통한-설치}

-   **ADB 명령어 사용**:
    1.  USB 디버깅 모드를 활성화합니다.
    2.  PC에 ADB 및 Fastboot 도구를 설치합니다.
    3.  APK 파일을 ADB 도구가 있는 폴더에 복사합니다.
    4.  명령 프롬프트를 열고, `adb install --bypass-low-target-sdk-block filename.apk` 명령어를 사용하여 설치합니다[2][5].


#### 3. APK 편집기를 통한 설치 {#3-dot-apk-편집기를-통한-설치}

-   **APK 편집기 사용**:
    1.  APK 편집기 앱을 설치합니다.
    2.  설치하려는 APK 파일을 선택하고, "target SDK version"을 23으로 변경합니다.
    3.  수정된 APK를 설치합니다[3].


#### 4. 직접 APK 설치 {#4-dot-직접-apk-설치}

-   **안전한 출처에서 APK 다운로드**:
    1.  신뢰할 수 있는 출처에서 APK 파일을 다운로드합니다.
    2.  설정에서 "알 수 없는 출처" 설치를 허용합니다.
    3.  다운로드한 APK 파일을 직접 설치합니다[4].

각 방법은 장단점이 있으니, 상황에 맞게 선택하세요.

-   Citations: [1] <https://github.com/termux/termux-app/issues/4120> [2] <https://www.youtube.com/watch?v=fapvDCwb-pQ> [3] <https://xdaforums.com/t/how-to-install-old-apk-on-android-14-without-pc.4645605/> [4] <https://www.fonedog.com/android-toolkit/install-incompatible-apps-on-android.html> [5] <https://www.youtube.com/watch?v=RPCqI9M49bA>

@user adb 연결해서 설치하려는데 다음 문제 발생. 확인 바람.

```text
➜ adb install emacs-30.1-21-arm64-v8a.apk
Performing Streamed Install
adb: failed to install emacs-30.1-21-arm64-v8a.apk: Failure [INSTALL_FAILED_SHARED_USER_INCOMPATIBLE: Reconciliation failed...: Reconcile failed: Package org.gnu.emacs has no signatures that match those in shared user com.termux; ignoring!]
```

@assistant `INSTALL_FAILED_SHARED_USER_INCOMPATIBLE` 오류는 `org.gnu.emacs` 패키지가 `com.termux` 공유 사용자 ID를 사용하려고 하는데, 서명이 일치하지 않아서 발생하는 문제입니다.

**해결 방법:**

1.  **기존 Emacs 삭제:** 기존에 설치된 Emacs가 있다면 완전히 삭제하고 다시 시도해 보세요. (adb uninstall org.gnu.emacs)

2.  **Termux Emacs 패키지 사용 (권장):** Termux 환경에서는 Termux 저장소에서 제공하는 Emacs 패키지를 사용하는 것이 가장 좋습니다. `pkg install emacs` 명령을 사용해서 설치하세요.

3.  **서명 확인:** APK 파일의 서명이 Termux와 호환되는지 확인하세요. 일반적으로 Termux 환경에 맞게 빌드된 APK가 아니면 문제가 발생할 수 있습니다.

4.  **다른 APK:** 다른 경로로 빌드된 Termux 전용 Emacs APK 파일을 찾아 설치해 보세요. (Termux Wiki 또는 관련 커뮤니티를 참고)

위에 나열된 방법들을 시도해 보세요. 대부분의 경우 기존 Emacs 삭제 후 Termux 패키지 관리자를 통해 설치하는 것이 가장 간단하고 안정적인 해결책입니다.
