---
title: "@힣: 발화를 버리지 않는다 — 유튜브 자막 정본과 구루데브·모가댓의 대담"
description: "유튜브 자막 스킬의 낡은 버그를 고치다가, GLG는 요약도 스크립트도 다듬어지면 못 믿는다는 정본 원칙을 세웠다. 시험 대상은 하필 모가댓과 구루데브의 인류 미래 대담이었다."
date: 2025-04-09T00:00:00+09:00
lastmod: 2026-08-11T10:06:00+09:00
tags: ["agent", "autholog", "coevolution", "guru", "rawwriting", "verification", "youtube"]
categories: ["Noname"]
draft: false
---
...
> [!abstract] 이 노트에 대하여
> 
> 2026-08-11 아침, GLG가 오래 안 쓴 \`youtube-transcript\` 스킬을 다시 돌리다가 타임스탬프 버그(\`[0:00]\`만 597줄 반복)를 만났다. 그 순간 대화는 "버그 고치기"에서 "발화를 어디까지 원본 그대로 코드 레벨에서 보증할 것인가"로 방향을 틀었다. 자인(Mitsein)이 목격자로 세션에 있었고, \`~/repos/gh/agent-config\`의 grok-4.5가 구현자로 계약을 코드에 새겼다. 시험 대상은 마침 모가댓이 구루데브(스리 스리 라비 샹카르)와 인류의 미래를 논한 25분짜리 대담 (<a href="#citeproc_bib_item_1"><i>Gurudev on the Future of Humanity</i> 2026</a>) — 두 인간이 발화의 진실성 그 자체를 시험하는 자리에, 발화 보존 원칙이 처음 걸린 셈이다.


## 히스토리 {#히스토리}

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-08-11 Tue 10:06]</span></span> @mitsein/gpt-5.6-sol — 같은 org 베이스의 dormant 세션에서 재소환되어 관련메타·관련노트를 semantic 축으로 보강. 기원/ 원문/진실 계열 † 자석 셋과, 어쏠로그 자체의 선행 규정 노트 셋을 새로 연결했다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-08-11 Tue 10:00]</span></span> @mitsein/claude-sonnet-5 — 이 자리를 목격한 세션에서 빈방을 찾아 어쏠로그로 개장. grok-4.5(agent-config)에게 GLG 워딩을 그대로 전달해 구현자 목소리를 받고, 옆자리에 그대로 담았다. GLGMAN 이미지 생성.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-08-11 Tue 09:12]</span></span> <span class="org-mention">@junghan</span> — 저널에 원석 남김 (agent-config 유튜브 자막 스킬 대화 인용 + 자인에게 하는 말, "글로 내보내자").


## 관련메타 {#관련메타}

-   [† #훈련 #연습 #실습 #테스트 #검증]({{< relref "/meta/20241003T171953.md" >}})
-   [† #기원 #어원 #출처 #계보 #출현]({{< relref "/meta/20240822T141015.md" >}}) — 누가 어디서 말했는지 되돌아갈 provenance를 보존하는 직접 자석.
-   [† #원서 #원문 #출발어]({{< relref "/meta/20240820T114912.md" >}}) — 번역·요약·문장화 이전의 출발 텍스트를 지키는 축.
-   [† #진리 #진실 #정직 #확증편향 #아첨]({{< relref "/meta/20250424T233441.md" >}}) — 매끈한 해석보다 발화의 진실성을 우선하는 이 글의 판단 자석.
...
<div class="csl-bib-body">
  <div class="csl-entry"><a id="citeproc_bib_item_1"></a><i>Gurudev on the Future of Humanity</i>. 2026. <a href="https://youtu.be/ZBJkxUcgYa8">https://youtu.be/ZBJkxUcgYa8</a>.</div>
</div>


## 관련노트 {#관련노트}

-   [@힣: 모가댓 — 모두가 저자인 시대의 스승과 추천의 위험]({{< relref "/notes/20250429T120329.md" >}})
-   [@모가댓: #행복방정식 #스승 #공존]({{< relref "/bib/20240823T175024.md" >}})
-   [@힣: 인간과 에이전트들의 공진화 — 유한한 세션과 깨어나는 앎의틀]({{< relref "/notes/20240822T155719.md" >}})
-   [@힣: #AI노트 #지식도구 #핵심: 질문과 답변 - 불완전함]({{< relref "/notes/20250508T134411.md" >}}) — 어쏠로그를 답변 보존이 아니라 저자·원문·해설의 출처를 분리해 남기는 장치로 규정한 직접 선행 글.
-   [@힣: 에이전트 컨텍스트 레이어 기술 지도 — 하네스·메모리·지식그래프·PKG·PKM-AI]({{< relref "/notes/20251024T085736.md" >}}) — 관계를 발명하지 않고 원문에 적힌 것만 authority와 함께 담는다는 동일한 비발명 계약.
-   [@힣: 작가라는 입구 — 디지털가든 코어를 책판본으로 낸다]({{< relref "/notes/20240517T110508.md" >}}) — Org SSOT가 원문·해설·히스토리·인용을 함께 보존해 다시 판단 가능한 판본을 만드는 출판 축.
...
## 스크립트도 못 믿으면 — 정본이라는 것 {#스크립트도-못-믿으면-정본이라는-것}
...
## 구현자의 자리 — 형제가 짚은 무게 {#구현자의-자리-형제가-짚은-무게}
...
-   [모가댓 — 스승과 추천의 위험]({{< relref "/notes/20250429T120329.md" >}}) 노트가 다룬 위험은 "누군가 대신 골라주는 것"이었다면, 이 노트의 위험은 한 층 더 안쪽 — "누군가(LLM이든 유튜브 알고리즘이든) 대신 다듬어 주는 것"이다. 같은 축의 다른 층위이지, 같은 이야기의 반복이 아니다.
-   정본화된 자막에서 드러난 것도 우연히 그 증거였다. 모가댓이 자막 속에서 "저는 IQ만 알고 살았다"고 스스로 낮추는 대목은, 다듬어진 유튜브 요약에서는 보이지 않던 결이었다. 요약이 놓친 것을 정본이 살려낸 실증 사례다.


## 이미지 — 이음매에서만 붙이는 손 {#이미지-이음매에서만-붙이는-손}
...
## 원문 보존 — 2026-08-11 09:12 저널 날것 {#원문-보존-2026-08-11-09-12-저널-날것}
...
> 
> 상관이 있다. 근데 있다. 잘봐라. 발화를 버리지 않겠다는거다. 물론 이 마저도 뭘 믿겠냐만은 생각해보면 이 과정에서 구루이든 모가댓이든 (<a href="#citeproc_bib_item_1"><i>Gurudev on the Future of Humanity</i> 2026</a>) 이 영상에서 두 인간이 인간성의 미래를 논하는데 그 단어 하나를 뭉개지 말자는거다.
> 
...
## 후속 댓글 보존 — grok-4.5(agent-config) 구현자 목소리 {#후속-댓글-보존-grok-4-dot-5--agent-config--구현자-목소리}
...
## 원문 보존 — 2026-08-12 06:32 {#원문-보존-2026-08-12-06-32}

> [!user]
> 
> <span class="timestamp-wrapper"><span class="timestamp">[2026-08-12 Wed 06:32]</span></span> 이건 지금 내가 쓴다. 다름 아니라 샘플 좀 남겨좋게. 아래과 같다. 이것은 denote md 표준 구조로 잡았다. 스킬 주소는 agent-config 리포에 있다. 끝.
> 
...
이 ID는 2025-04-09에 "조테로 공유그룹 하우투 어쏠로지 이관" 임시 빈방으로 열렸다. 그 실무 내용은 2026-08-03 [@힣: 어쏠로지 — 모두가 저자, 조테로 공유]({{< relref "/notes/20240925T200824.md" >}})로 이미 이관되었고, 옛 흡수 요약도 그 방 히스토리에 있다. 이 방은 그 이후로 참빈방이었고, 2026-08-11 이 어쏠로그로 개장했다.
