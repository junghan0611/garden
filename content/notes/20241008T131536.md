---
title: "#생성기 #이맥스 #닷파일"
description: "이맥스 스타터키트와 설정 생성기를 비교하며 닷파일 생성기의 의미를 살피는 노트이다. 실제 활용 흐름을 담았다."
date: 2024-10-08T13:15:00+09:00
lastmod: 2024-12-10T13:27:00+09:00
tags: ["dotfiles", "emacs", "generator", "notes"]
categories: ["Noname"]
draft: false
---

> [!abstract] 이 노트에 대하여
> 
> Emacs 설정 생성기와 스타터키트가 어떤 차이로 초보자 진입을 돕는지 간단히 정리한다. 수작업 설정과 자동 생성 사이에서 무엇이 더 간단하고 유연한지 생각하게 만든다.

<span class="timestamp-wrapper"><span class="timestamp">[2024-10-08 Tue 13:17]</span></span> 놀랍다. 간단하긴 해도 나름의 구성을 만들어 준다. 기능이 협소하다. 어쩔 수 없다. 자유도가 필요한가? 활용방안을 보여주고 잡아주는게 더 나은 방식이 아닐까? 아무튼 그렇다. [#닷파일: #둠이맥스 #스타터]({{< relref "/notes/20240915T235008.md" >}}) 생각해보니 그렇다.


## Emacs starter kit <span class="org-hashtag">#configurationgenerator</span> <span class="org-hashtag">#닷파일생성기</span> : {#emacs-starter-kit-configurationgenerator-닷파일생성기}

(<a href="#citeproc_bib_item_2">Graham Marlow n.d.</a>)

이게 더 간단하긴하다.

-   [contacts::Graham Marlow - mgmarlow - mgmarlow.com]({{< relref "/meta/20230303T030300.md#h-27c3113d-578c-4f3a-b271-6347f020fcec" >}})


## Emacs Configuration Generator <span class="org-hashtag">#configurationgenerator</span> <span class="org-hashtag">#닷파일생성기</span> <span class="org-hashtag">#이맥스</span> : {#emacs-configuration-generator-configurationgenerator-닷파일생성기-이맥스}

(<a href="#citeproc_bib_item_1">“Emacs Configuration Generator #Configurationgenerator #닷파일생성기 #이맥스” n.d.</a>)


## Related-Notes {#related-notes}

-   [#생성기]({{< relref "/meta/20241008T131455.md" >}})

## BIBLIOGRAPHY

<div class="csl-bib-body">
  <div class="csl-entry"><a id="citeproc_bib_item_1"></a>“Emacs Configuration Generator #Configurationgenerator #닷파일생성기 #이맥스.” n.d. Accessed October 8, 2024. <a href="https://emacs.amodernist.com/">https://emacs.amodernist.com/</a>.</div>
  <div class="csl-entry"><a id="citeproc_bib_item_2"></a>Graham Marlow. n.d. “Emacs Starter Kit #Configurationgenerator #닷파일생성기.” Accessed October 8, 2024. <a href="https://emacs-config-generator.fly.dev/">https://emacs-config-generator.fly.dev/</a>.</div>
</div>

