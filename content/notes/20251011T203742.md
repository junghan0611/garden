...
description: "NixOS와 Home-Manager로 애플리케이션과 사용자 환경의 재현성을 확보하는 전략을 정리한 노트이다."
date: 2025-10-11T20:37:00+09:00
lastmod: 2025-10-11T20:37:00+09:00
tags: ["llmlog", "nixos", "reproducibility", "homemanager", "configuration"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-10-11 Sat 20:37]</span></span> 연휴

<!--listend-->

-   [#리눅스 #한글 #입력기 kime ibus]({{< relref "/notes/20240325T073822.md" >}})


## NixOS Home-Manager 애플리케이션 재현성 전략 {#nixos-home-manager-애플리케이션-재현성-전략}


### 핵심 인사이트: 재현성의 진정한 의미 {#핵심-인사이트-재현성의-진정한-의미}


#### 일반적인 재현성 (시스템 레벨) {#일반적인-재현성--시스템-레벨}
...
#### 완전한 재현성 (애플리케이션 + 설정) {#완전한-재현성--애플리케이션-plus-설정}
...
### 기존 방식의 한계: Stow {#기존-방식의-한계-stow}


#### GNU Stow의 접근법 {#gnu-stow의-접근법}
...
### NixOS Home-Manager 방식: 완전한 재현성 {#nixos-home-manager-방식-완전한-재현성}
...
#### Home-Manager의 접근법 {#home-manager의-접근법}
...
##### 1단계: 설정 파일 선언 (home-manager.nix) {#1단계-설정-파일-선언--home-manager-dot-nix}
...
##### 3단계: 백업 시스템 (lib/mksystem.nix) {#3단계-백업-시스템--lib-mksystem-dot-nix}
...
### 구현 사례: 재현성 확보한 애플리케이션들 {#구현-사례-재현성-확보한-애플리케이션들}


#### fcitx5 (한글 입력기) {#fcitx5--한글-입력기}
...
#### Flameshot (스크린샷 도구) {#flameshot--스크린샷-도구}
...
### 재현성 확보 표준 워크플로우 {#재현성-확보-표준-워크플로우}


#### Step 1: 기존 설정 파일 발견 {#step-1-기존-설정-파일-발견}
...
#### Step 2: Nix 설정으로 변환 {#step-2-nix-설정으로-변환}
...
#### Step 3: 기존 백업 정리 (필요시) {#step-3-기존-백업-정리--필요시}
...
#### Step 5: 모든 머신에 적용 {#step-5-모든-머신에-적용}
...
#### 1. 단일 진실 소스 (Single Source of Truth) {#1-dot-단일-진실-소스--single-source-of-truth}
...
#### 2. 충돌 불허 (Zero Tolerance for Conflicts) {#2-dot-충돌-불허--zero-tolerance-for-conflicts}
...
#### 재현성 확보 대상 애플리케이션 (예시) {#재현성-확보-대상-애플리케이션--예시}
...
### 결론: 재현성의 단계 {#결론-재현성의-단계}


#### Level 0: 수동 설치 {#level-0-수동-설치}
...
#### Level 1: 스크립트 자동화 {#level-1-스크립트-자동화}
...
#### Level 2: Dotfiles + Stow {#level-2-dotfiles-plus-stow}
...
#### Level 3: NixOS + Home-Manager (우리가 도달한 수준) {#level-3-nixos-plus-home-manager--우리가-도달한-수준}
