---
title: "@claude-memory @Denote 마이그레이션 전략"
description: "claude-memory를 Denote 형식의 ~/org 지식베이스로 옮길지 고민한 마이그레이션 전략 노트이다."
date: 2025-10-16T07:52:00+09:00
lastmod: 2025-10-16T07:52:00+09:00
tags: ["llmlog", "migration", "claudememory", "denote", "urgent", "digitalgarden"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-10-16 Thu 07:52]</span></span> 고민
-   [[최종 결정] 3-Tier 지식베이스 아키텍처 (마이그레이션 보류)]({{< relref "/notes/20251016T091521.md" >}})
-   [에이전트 지식베이스 아키텍처 ULTRATHINK]({{< relref "/notes/20251016T080208.md" >}})


## claude-memory → ~/org/ Denote 마이그레이션 전략 {#claude-memory-org-denote-마이그레이션-전략}


### 🎯 미션: 디지털 가든 퍼블리시 준비 {#미션-디지털-가든-퍼블리시-준비}
...
### 📊 현황 분석 (2025-10-16 07:52) {#현황-분석--2025-10-16-07-52}


#### claude-memory 구조 (PARA 방법론) {#claude-memory-구조--para-방법론}
...
#### ~/org/ 구조 (Denote 기반) {#org-구조--denote-기반}
...
#### Phase 1: 파일 포맷 변환 (MD → ORG) ⚡ URGENT {#phase-1-파일-포맷-변환--md-org--urgent}
...
##### 디렉토리 맵핑 (PARA → Denote) {#디렉토리-맵핑--para-denote}
...
#### Phase 2: 변환 스크립트 작성 {#phase-2-변환-스크립트-작성}


##### 스크립트 개요: `migrate_memory_to_org.py` {#스크립트-개요-migrate-memory-to-org-dot-py}
...
#### Phase 3: 검증 &amp; 클린업 {#phase-3-검증-and-클린업}
...
#### Phase 4: 디지털 가든 퍼블리시 {#phase-4-디지털-가든-퍼블리시}


##### Quartz 설정 확인 {#quartz-설정-확인}
...
### ⚡ 긴급 실행 체크리스트 (출근 직후) {#긴급-실행-체크리스트--출근-직후}
...
#### 왜 이 마이그레이션이 필요한가? {#왜-이-마이그레이션이-필요한가}


##### 단일 진실의 원천 (Single Source of Truth) {#단일-진실의-원천--single-source-of-truth}
...
##### 리스크 3: 시간 부족 (출근 전) {#리스크-3-시간-부족--출근-전}
...
-   [시간과정신의방 프로젝트]({{< relref "/notes/20251015T093311.md" >}})
...
#### "Config as Being" 실현 {#config-as-being-실현}
