---
title: "§andenken 이맥스 임베딩 검색"
description: "andenken 시멘틱 메모리 백엔드와 doomemacs-config 측 얇은 이맥스 클라이언트 설계를 정리한 노트."
date: 2025-02-14T14:56:00+09:00
lastmod: 2026-05-28T17:00:00+09:00
tags: ["ai", "andenken", "embedding", "emacs", "rag", "semanticsearch"]
categories: ["Noname"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu 17:00]</span></span> sessions retrieval 진단 단락 추가. md 트랙은 일단 안 봄(글 형식이라 기대치 다름), sessions 한정으로 백엔드 품질 자리 잡힘. andenken 담당자가 NEXT.md 2e (session-as-unit windowed view) 진입점으로 잡음. boundary 단락 박음 — andenken=단축, recall skill= 다축 합성.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu 16:00]</span></span> time-axis 명령 추가 (`-sessions-today`, `-sessions-this-week`). 기존 `my/denote-links-today` / `-this-week` 패턴 그대로. preview 버퍼 + 키맵 (`RET` `/org / ~o` md / `q` 닫기) 추가.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu 15:00]</span></span> andenken을 백엔드로 확정. doomemacs-config 측 얇은 클라이언트(`lisp/andenken-config.el`) 추가. 노트 방향을 "khoj 검토"에서 "이맥스 인터페이스 설계"로 재정렬.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu 14:05]</span></span> 수정할거야.
...
## 결정 — andenken 백엔드, 이맥스가 얇은 클라이언트 {#결정-andenken-백엔드-이맥스가-얇은-클라이언트}
...
**인터페이스 더 안 늘림.** <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu]</span></span> 결정. "복잡하면 사용하지 않겠다는 것" — 사용자 명시.
...
## <span class="timestamp-wrapper"><span class="timestamp">[2026-05-28 Thu 17:00]</span></span> sessions retrieval 진단 — 경계 다시 잡기 {#sessions-retrieval-진단-경계-다시-잡기}
...
### 분신 진단 → andenken 담당자 정정 {#분신-진단-andenken-담당자-정정}
...
### Boundary (잠금 — GPT 리뷰 전에 박음) {#boundary--잠금-gpt-리뷰-전에-박음}
...
### andenken 측 진행 (담당자 SSOT 는 `andenken/NEXT.md`) {#andenken-측-진행--담당자-ssot-는-andenken-next-dot-md}
...
### 임베딩 자체가 별로일 가능성 {#임베딩-자체가-별로일-가능성}
...
## [|2025-03-28 Fri 09:03|]({{< relref "/journal/20250324T000000.md#h-2025-03-28" >}}) {#2025-03-28-fri-09-03-h-2025-03-28}
