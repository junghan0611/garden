---
title: "agent-shell + OpenCode 커스텀 가이드: ACP 프로토콜 통합"
description: "agent-shell과 OpenCode를 ACP 프로토콜로 직접 통합하는 방법을 정리한 노트이다."
date: 2025-11-21T13:14:00+09:00
lastmod: 2025-11-21T13:14:00+09:00
tags: ["llmlog", "agentshell", "opencode", "acp", "emacs", "customization"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-11-21 Fri 13:20]</span></span> 에이전트에게 부탁


## agent-shell + OpenCode 커스텀 가이드 {#agent-shell-plus-opencode-커스텀-가이드}
...
### 🔧 OpenCode 아키텍처 {#opencode-아키텍처}


#### 1. ACP 에이전트 (acp/agent.ts) {#1-dot-acp-에이전트--acp-agent-dot-ts}
...
##### 이벤트 구독 (setupEventSubscriptions) {#이벤트-구독--setupeventsubscriptions}
...
#### 2. 권한 시스템 (permission/index.ts) {#2-dot-권한-시스템--permission-index-dot-ts}


##### 세션 레벨 권한 관리 {#세션-레벨-권한-관리}
...
#### 3. 서브에이전트 시스템 (agent/agent.ts) {#3-dot-서브에이전트-시스템--agent-agent-dot-ts}
...
##### 커스텀 에이전트 설정 (opencode.jsonc) {#커스텀-에이전트-설정--opencode-dot-jsonc}
...
#### 4. 프로바이더 시스템 (provider/provider.ts) {#4-dot-프로바이더-시스템--provider-provider-dot-ts}


##### 현재 사용 가능한 프로바이더 {#현재-사용-가능한-프로바이더}
...
#### 1. 코드 리팩토링 워크플로우 {#1-dot-코드-리팩토링-워크플로우}
...
#### 3. 복잡한 버그 수정 워크플로우 {#3-dot-복잡한-버그-수정-워크플로우}
...
#### OpenCode 설정 (~/.config/opencode/ 또는 프로젝트 _.opencode_) {#opencode-설정--dot-config-opencode-또는-프로젝트-dot-opencode}
...
### 🛠️ agent-shell 커스터마이징 {#️-agent-shell-커스터마이징}


#### Elisp 통합 함수 {#elisp-통합-함수}
...
#### 3. MCP 서버 동적 로딩 테스트 {#3-dot-mcp-서버-동적-로딩-테스트}
...
#### 단계 1: agent-shell 기본 통합 {#단계-1-agent-shell-기본-통합}
