---
title: "#템플릿: 조직모드 예제 - 각주 수식 인용 레이텍 - 수정 대기중"
description: "조직모드에서 각주, 수식, 인용, lastmod 같은 퍼블리시 요소를 점검하는 예제 템플릿 모음."
date: 2023-06-05T12:18:00+09:00
lastmod: 2025-02-26T00:00:00+09:00
tags: ["examples", "hugo", "markdown", "orgmode", "templates"]
categories: ["Noname"]
draft: false
math: true
---
...
,#+begin_description Org 파일로 Hugo 블로깅 위한 예제 (Org -&gt; Markdown) ,#+end_description
...
<span class="timestamp-wrapper"><span class="timestamp">[2026-09-20 Sun 08:36]</span></span> 추가 C-c C-, 단축키 처음 알았네

@assistant

```markdown
Org 모드는 #+BEGIN_...와 #+END_... 구조로 감싸서 사용하는 다양한 내장 블록(Blocks) 구조를 제공합니다. 텍스트 서식 지정, 코드 실행, 레이아웃 변경 등 용도에 따라 자주 쓰이는 핵심 블록들을 정리해 드릴게요. [1, 2]
------------------------------
## 1. 인용 및 강조 블록

- QUOTE (인용구)
- 다른 문서의 글을 인용할 때 사용합니다. 내보내기를 하면 좌우 여백이 들어가며 들여쓰기된 형태로 깔끔하게 정렬됩니다.
   - VERSE와 차이점: 줄 바꿈을 유지하지 않고 일반 문단처럼 하나로 합칩니다. [1, 3]
...
- CENTER (가운데 정렬)
- 블록 내부의 텍스트나 이미지를 화면 중앙에 정렬합니다. [1]
...
- SRC (소스 코드 블록)
- Org 모드의 핵심 기능 중 하나입니다. 특정 프로그래밍 언어의 문법 강조(Syntax Highlighting)를 지원하며, Emacs 안에서 코드를 즉시 실행(C-c C-c)하고 그 결과까지 문서에 바로 삽입할 수 있습니다. [1, 4, 5]

#+BEGIN_SRC python
def hello():
    return "Hello Org-mode!"
#+END_SRC

- EXAMPLE (고정폭 예시)
- 소스 코드는 아니지만 터미널 명령어 출력 창, 로그 파일 데이터 같은 raw 텍스트를 고정폭 글꼴(Monospace)로 가감 없이 보여주고 싶을 때 씁니다. [1]
...
## 3. 문서 내보내기 전용 블록 (Export)

- EXPORT (특정 포맷 전용)
- HTML이나 PDF(LaTeX) 등 특정 파일 형식으로 내보낼 때만 포함될 내용을 작성합니다. Org 모드 문법이 아닌 대상 포맷의 문법을 직접 쓸 수 있습니다. [1, 6]

#+BEGIN_EXPORT html
<div class="alert alert-info">이것은 HTML 전용 안내창입니다.</div>
#+END_EXPORT

- COMMENT (주석 블록)
- 작성 중인 문서 내용 중 메모나 초안을 감추고 싶을 때 씁니다. 에디터에서는 보이지만 HTML, PDF 등으로 문서를 내보낼 때는 완전히 제외됩니다. [1]

#+BEGIN_COMMENT
이 부분은 나중에 수정할 예정이므로 출력되지 않게 숨겨둡니다.
#+END_COMMENT

------------------------------
💡 꿀팁 (블록 쉽게 만드는 단축키)
Emacs에서 블록을 일일이 타이핑하기 귀찮을 때는 영역(Region)을 지정한 뒤 C-c C-,를 누르면 메뉴가 뜹니다. [4]

- q를 누르면 QUOTE
- s를 누르면 SRC
- v를 누르면 VERSE 블록이 자동으로 감싸집니다. [1]


[1] [https://orgmode.org](https://orgmode.org/manual/Working-with-Source-Code.html)
[2] [https://orgmode.org](https://orgmode.org/manual/Blocks.html)
[3] [https://orgmode.org](https://orgmode.org/manual/Paragraphs.html)
[4] [https://orgmode.org](https://orgmode.org/manual/Structure-of-Code-Blocks.html)
[5] [https://org-babel.readthedocs.io](https://org-babel.readthedocs.io/en/latest/eval/)
[6] [https://github.com](https://github.com/nhoffman/org-export)
```
...
## <span class="org-hashtag">#lastmod</span> <span class="org-hashtag">#timestamp</span> {#lastmod-timestamp}

<span class="timestamp-wrapper"><span class="timestamp">[2024-11-27 Wed 11:55]</span></span>
...
## Math Typesetting 수식 입력 {#h-c078e2a6-6a7e-431b-a81f-55286e94edd3}
...
{{< figure src="/home/junghan/org/notes/20231231_081558_screenshot.png" caption="<span class=\"figure-number\">Figure 4: </span>org-attach test" width="100%" >}}
