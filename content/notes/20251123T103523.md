---
title: "Steve Yegge #분석 #바이브코딩 #인터뷰 정리"
description: "Steve Yegge 인터뷰를 통해 바이브코딩과 ACP 도구 본질을 분석한 정리 노트이다."
date: 2025-11-23T10:35:00+09:00
lastmod: 2025-11-23T10:35:00+09:00
tags: ["agent", "bib", "interview", "llmlog", "vibecoding", "yegge"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-11-23 Sun 15:40]</span></span> 만든것
    -   [@스티브예기 steveyegge §beads §efrit #바이브코딩]({{< relref "/bib/20251123T152633.md" >}})


## Steve Yegge Beads 에이전트 메모리 시스템 분석 {#steve-yegge-beads-에이전트-메모리-시스템-분석}


### 배경: ACP 생태계 탐구 {#배경-acp-생태계-탐구}

ACP(Agent Client Protocol) 도구들의 본질을 탐구하던 중 Steve Yegge의 Beads 프로젝트 발견.


#### ACP 생태계 현황 {#acp-생태계-현황}
...
### Steve Yegge는 누구인가? {#steve-yegge는-누구인가}


#### 기본 프로필 (Wikipedia/블로그 검증됨) {#기본-프로필--wikipedia-블로그-검증됨}
...
#### 왜 Emacs 패키지를 만드는가? {#왜-emacs-패키지를-만드는가}
...
### Beads 프로젝트 분석 {#beads-프로젝트-분석}


#### 해결하려는 문제: 에이전트의 기억상실 {#해결하려는-문제-에이전트의-기억상실}
...
#### 핵심 아키텍처: Git을 분산 데이터베이스로 {#핵심-아키텍처-git을-분산-데이터베이스로}
...
### Steve Yegge의 AI 에이전트 생태계 {#steve-yegge의-ai-에이전트-생태계}


#### 세 프로젝트 관계도 {#세-프로젝트-관계도}
...
#### VC의 AI Supervised Issue Workflow {#vc의-ai-supervised-issue-workflow}
...
### 힣's -config 생태계와의 통합 가능성 {#힣-s-config-생태계와의-통합-가능성}
...
#### Beads가 들어갈 수 있는 지점 {#beads가-들어갈-수-있는-지점}
...
#### Denote vs Beads 비교 {#denote-vs-beads-비교}
...
### VC의 코드 품질 관리 체계 분석 (2025-11-22 추가) {#vc의-코드-품질-관리-체계-분석--2025-11-22-추가}

Steve Yegge가 "에이전트에게 개발을 일임"하면서도 코드 퀄리티를 유지하는 방법.


#### Quality Gates 시스템 {#quality-gates-시스템}
...
#### Pre-Landing Review Protocol (CLAUDE.md:68-147) {#pre-landing-review-protocol--claude-dot-md-68-147}
...
#### 점진적 Linting 전략 (LINTING.md) {#점진적-linting-전략--linting-dot-md}
...
#### AI 코드 리뷰 예시 (CODE_REVIEW_vc-5b22.md) {#ai-코드-리뷰-예시--code-review-vc-5b22-dot-md}
...
#### 코드 리뷰 자동화 (code_review_check.go) {#코드-리뷰-자동화--code-review-check-dot-go}
...
#### 핵심 통찰: "Safety Nets" {#핵심-통찰-safety-nets}
...
## Steve Yegge Vibe Coding 인터뷰 요약 {#steve-yegge-vibe-coding-인터뷰-요약}
...
### Steve Yegge 소개 (인터뷰어 기준) {#steve-yegge-소개--인터뷰어-기준}
...
#### 1. 프로그래머는 이제 "팀 리드"다 {#1-dot-프로그래머는-이제-팀-리드-다}
...
#### 2. IDE는 죽었다, CLI가 미래다 {#2-dot-ide는-죽었다-cli가-미래다}
...
#### 3. Vibe Coding의 중독성 {#3-dot-vibe-coding의-중독성}
...
#### 4. 컨텍스트 윈도우와 작업 분해 {#4-dot-컨텍스트-윈도우와-작업-분해}
...
#### 5. 모놀리스 회사들은 망한다 {#5-dot-모놀리스-회사들은-망한다}
...
#### 6. 주니어 엔지니어의 새로운 역할 {#6-dot-주니어-엔지니어의-새로운-역할}
...
### Amazon vs Google 비교 {#amazon-vs-google-비교}
...
## 타임라인: Vibe Coding Is The Only Future  - Steve Yegge {#타임라인-vibe-coding-is-the-only-future-steve-yegge}
...
### 0:00--5:30 --- 오프닝, 비선형 커리어 소개 {#오프닝-비선형-커리어-소개}
...
### 5:30--13:00 --- Amazon vs Google, "중력 블랙홀" 회사들 {#amazon-vs-google-중력-블랙홀-회사들}
...
### 13:00--17:30 --- Google+ 플랫폼 랜트 유출 {#google-플랫폼-랜트-유출}
...
### 17:30--25:00 --- AI 발전, 데이터, 컨텍스트 윈도우, 작업 분해 {#ai-발전-데이터-컨텍스트-윈도우-작업-분해}
...
### 25:00--30:30 --- Vibe Coding vs Prompt Engineering, IDE vs CLI {#vibe-coding-vs-prompt-engineering-ide-vs-cli}
...
### 30:30--36:00 --- "당신은 이제 팀 리드" : 역할 전환 {#당신은-이제-팀-리드-역할-전환}
...
### 36:00--41:00 --- 주니어 아래의 새로운 층, 비개발자도 Vibe Coder로 {#주니어-아래의-새로운-층-비개발자도-vibe-coder로}
...
### 41:00--45:30 --- Vibe Coding의 감각과 중독성, Wyvern 부활 {#vibe-coding의-감각과-중독성-wyvern-부활}
...
### 45:30--48:30 --- 프로그래밍의 피로, AI의 역할, 경험 중심 사고 {#프로그래밍의-피로-ai의-역할-경험-중심-사고}
...
### 48:30--끝 --- 라피드 파이어: 과대평가/과소평가 주제들 {#끝-라피드-파이어-과대평가과소평가-주제들}
...
### 이 대담이 에이전트 워크플로에 주는 시사점 {#이-대담이-에이전트-워크플로에-주는-시사점}
