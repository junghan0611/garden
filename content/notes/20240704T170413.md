...
description: "둠이맥스에서 바꿔 둔 키바인딩과 관련 설정 조각을 한데 모아 두는 기록 노트. 관련 내용을 정리한 기록."
...
### [|2025-07-13 Sun 15:48|]({{< relref "/journal/20250707T000000.md#2025-07-13-sun" >}}) GPTEL 트렌지언트 {#2025-07-13-sun-15-48-2025-07-13-sun--gptel-트렌지언트}


#### @user 아래 gptel 코드를 보면, "M+"를 입력하라고 하는데 {#user-아래-gptel-코드를-보면-m-plus-를-입력하라고-하는데}
...
#### @user 실제로 gptel 패키지의 transient에서 저 키 조합이 입력이 안된다. 다른 키 바인딩으로 충돌이 나는 것 같은데 확인하고 해결하는 방법을 알려줘 {#user-실제로-gptel-패키지의-transient에서-저-키-조합이-입력이-안된다-dot-다른-키-바인딩으로-충돌이-나는-것-같은데-확인하고-해결하는-방법을-알려줘}

@assistant 키 바인딩 충돌 문제를 해결하기 위한 방법은 다음과 같습니다:


#### 1. 현재 키 바인딩 확인하기 {#1-dot-현재-키-바인딩-확인하기}
...
#### 2. transient 메뉴 내 키 바인딩 변경하기 {#2-dot-transient-메뉴-내-키-바인딩-변경하기}
...
#### @user 확인해 보니 다음과 같이 나오는데 이유가 무엇일까? {#user-확인해-보니-다음과-같이-나오는데-이유가-무엇일까}
