---
title: "@homeagent-config ML 파이프라인 도구세트 운영전략"
description: "ML 파인튜닝 파이프라인을 운영 인프라 관점에서 설계하기 위한 도구세트 전략을 정리한 노트이다."
date: 2026-02-09T18:30:00+09:00
lastmod: 2026-02-12T00:00:00+09:00
tags: ["doltgres", "homeagent", "llmlog", "mlops", "nixos", "pipeline", "temporal"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-11 Wed 10:43]</span></span> <span class="org-mention">@junghan</span> — temporal 관점에서 하나 더 맥락을 강화하려고 한다. 별도 문서를 만들어야 할 것 같은데?
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-02-12 Thu 17:30]</span></span> 업데이트


## HomeAgent ML 파이프라인 도구세트 운영전략 {#homeagent-ml-파이프라인-도구세트-운영전략}

NixOS GPU 클러스터에서 ML 파인튜닝 파이프라인을 운영하기 위한 도구세트 결정 과정. 알고리즘이 아닌 **운영 인프라 관점** — 도구 선택, 에이전트 협업, 상태 모니터링에 집중한다.


### 배경: 왜 이 파이프라인이 필요한가 {#배경-왜-이-파이프라인이-필요한가}
...
### 현재 인프라 상태 (2026-02-09) {#현재-인프라-상태--2026-02-09}
...
### Q1 확정: 실험 트래킹 → wandb SaaS {#q1-확정-실험-트래킹-wandb-saas}
...
#### 결정: wandb SaaS로 시작 {#결정-wandb-saas로-시작}
...
### Q2 확정: 파이프라인 오케스트레이션 → Temporal Python Worker {#q2-확정-파이프라인-오케스트레이션-temporal-python-worker}
...
#### 조사 결과: Temporal Python SDK {#조사-결과-temporal-python-sdk}
...
#### 결정: Temporal Python Worker {#결정-temporal-python-worker}
...
### Q3 확정: 데이터셋 버전 관리 → Doltgres + NFS {#q3-확정-데이터셋-버전-관리-doltgres-plus-nfs}
...
#### 비교: Doltgres vs DVC vs git-lfs {#비교-doltgres-vs-dvc-vs-git-lfs}
...
#### 결정: Doltgres (데이터셋) + NFS (아티팩트) {#결정-doltgres--데이터셋--plus-nfs--아티팩트}
...
#### 리스크 2: Beta 소프트웨어 의존 {#리스크-2-beta-소프트웨어-의존}
...
### 다음 행동 (Action Items) {#다음-행동--action-items}
