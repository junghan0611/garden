---
title: "#매뉴얼: #둠이맥스 닷파일 이맥스리스프 코딩 컨벤션"
description: "Doom Emacs 닷파일을 장기적으로 정리하고 모듈화하기 위한 Emacs Lisp 코딩 컨벤션과 참조 문서."
date: 2024-04-04T10:10:00+09:00
lastmod: 2026-06-09T10:00:00+09:00
tags: ["coding", "convention", "doomemacs", "dotfiles", "emacslisp", "manual"]
categories: ["Noname"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-09 Tue 10:00]</span></span> vanilla-first 테스트 게이트(Tier A/B/C) 원칙 추가.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-06-08 Mon 08:28]</span></span> 기존 Doom \`lisp/demos.org\` 덤프 성격의 노트를 지우고, 닷파일 코딩 컨벤션 문서로 재작성.
-   <span class="timestamp-wrapper"><span class="timestamp">[2024-04-04 Thu 10:10]</span></span> 생성.
...
## Vanilla-first 테스트 게이트 {#vanilla-first-테스트-게이트}
...
### Tier A — vanilla logic {#tier-a-vanilla-logic}
...
### Tier B — Doom configuration {#tier-b-doom-configuration}
...
### Tier C — package-dependent integration {#tier-c-package-dependent-integration}
...
### Doom module / config guide {#doom-module-config-guide}
...
### Doom compat module {#doom-compat-module}
...
### agent-shell contributing style {#agent-shell-contributing-style}
...
### General Emacs Lisp style {#general-emacs-lisp-style}
...
### Hook / advice / strategy suffix {#hook-advice-strategy-suffix}
...
## Doom v3-aware rules {#doom-v3-aware-rules}


### 새 코드에서 compat shim 금지 {#새-코드에서-compat-shim-금지}
...
### grep-ability가 필요하면 local predicate {#grep-ability가-필요하면-local-predicate}
...
## Data and library preference {#data-and-library-preference}


### 작은 구조화 데이터는 alist {#작은-구조화-데이터는-alist}
...
### alist access는 \`map.el\` {#alist-access는-map-dot-el}
...
### list processing은 \`seq.el\` {#list-processing은-seq-dot-el}
...
### string helper는 \`subr-x\` {#string-helper는-subr-x}
...
### single-branch \`if\` 금지 {#single-branch-if-금지}
...
## Customization policy {#customization-policy}
...
### docstring에는 입력/출력 예시 {#docstring에는-입력-출력-예시}
...
## Package/config boundary {#package-config-boundary}


### Doom config 안에 남길 코드 {#doom-config-안에-남길-코드}
