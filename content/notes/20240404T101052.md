...
When <kbd>newline</kbd> is passed a numerical prefix argument (`C-u 5 M-x newline`), it inserts N newlines. We can use <kbd>cmd!!</kbd> to easily create a keybinds that bakes in the prefix arg into the command call:
...
These are side-by-side comparisons, showing how to bind keys with and without <kbd>map!</kbd>:
