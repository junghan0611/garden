---
title: "@힣: Self-documenting CLI — 문서와 실행을 잇는 인간·에이전트 공통 인터페이스"
description: "프로젝트의 설명서와 실행기를 하나로 묶어 인간은 대화형 메뉴로, 에이전트는 안정된 인자로 같은 기능에 접근하게 하는 Self-documenting CLI의 변하지 않는 원칙을 정리한다."
date: 2026-02-12T10:31:00+09:00
lastmod: 2026-07-30T22:25:00+09:00
tags: ["autholog", "automation", "human", "interface", "scripts", "workflow"]
categories: ["Noname"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-30 Thu 20:10]</span></span> @pi@oracle (ACP Sonnet5, authologplay) — 세계 이미지와 `:PROMPT:` 헤딩을 처음 채웠다. Org SSOT 메타문서(20251021T105353)의 "한 파이프라인의 서로 다른 층"이라는 문장을 그대로 시각 컨셉으로 삼아, 하나의 상류(정본)가 두 문(인간 메뉴/에이전트 인자)으로 갈라졌다가 같은 웅덩이로 다시 합쳐지는 장면을 GLGMAN Universe로 그렸다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-23 Thu 21:14]</span></span> 2월의 구현 스냅샷과 LLM 대화 덤프를 걷어내고, 인간·에이전트 공통 인터페이스라는 변하지 않는 원칙으로 전면 수선했다. 최신 명령 목록과 Bash 구현은 각 리포의 `run.sh --help` 및 문서를 SSOT로 삼는다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-02-12 Thu 17:21]</span></span> `memex-kb/run.sh` 의 Registry + Introspection 구현 경험, Bash `set -euo pipefail` 함정, 사람용 메뉴와 에이전트용 인자 인터페이스를 한 노트로 모았다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-02-12 Thu 10:31]</span></span> “설명서 + 실행기 for 인간/에이전트”라는 질문에서 생성했다.


## 관련메타 {#관련메타}

-   [† #워크플로우 #자동화]({{< relref "/meta/20231012T121700.md" >}}) — 반복 가능한 실행과 운영 흐름.
-   [† #조직모드 §org-mode]({{< relref "/meta/20230831T154800.md" >}}) — 문서·코드·실행 절차의 정본.
-   [† #human #인간 #휴먼 #사람]({{< relref "/meta/20250424T230543.md" >}}) — 메뉴와 설명을 통해 실행의 의미를 읽는 사용자.
-   [† #협업 #협력 #집단지성 #코웍]({{< relref "/meta/20241006T090050.md" >}}) — 같은 기능을 서로 다른 손으로 호출하는 공통면.
-   [† #비서 #도우미 #튜터 #에이전트 #에이전틱 #챗봇]({{< relref "/meta/20250512T132547.md" >}}) — 안정된 명령과 도움말을 읽는 실행 주체.


## 관련노트 {#관련노트}

-   [@힣: AX 문서 협업과 인간 존중 — 자동화할 것과 시간을 내어 답할 것]({{< relref "/notes/20250224T172456.md" >}}) — CLI가 자동화할 형식과 인간에게 남겨 둘 관계의 책임을 구분한다.
-   [@힣: Org SSOT 메타문서 — 책의 미래와 인터랙티브 프로젝트 지식베이스]({{< relref "/notes/20251021T105353.md" >}}) — Org 정본과 실행 가능한 문서 파이프라인의 상위 문서론.
-   [§memex-kb 제안서 문서 변환 메타포멧]({{< relref "/botlog/20260306T130726.md" >}}) — `run.sh build` 가 실제 제안서 변환과 전달 흐름에서 사용된 사례.
-   [@힣: 지식베이스 중심 이맥스 리터레이트 코딩 tangle detangle]({{< relref "/notes/20250714T172141.md" >}}) — 코드와 문서를 같은 지식베이스에서 다루는 선행 축.
-   [◊denote 지식베이스 프로토콜 — 인간·에이전트 공존 인터페이스]({{< relref "/botlog/20260308T091235.md" >}}) — 파일·링크·읽기·쓰기에서 같은 규약을 쓰는 더 넓은 공통면.
...
## Org SSOT와 CLI의 역할은 다르다 {#org-ssot와-cli의-역할은-다르다}

이 노트와 [Org SSOT 메타문서]({{< relref "/notes/20251021T105353.md" >}})는 닮았지만 중복이 아니다.
...
## 인간과 에이전트가 같은 기능을 쓴다는 것 {#인간과-에이전트가-같은-기능을-쓴다는-것}
...
## 문서 협업과 인간존중의 경계 {#문서-협업과-인간존중의-경계}
...
[AX 문서 협업과 인간 존중]({{< relref "/notes/20250224T172456.md" >}})이 세운 경계는 이 실행기에도 적용된다.
...
## 2026-02 구현 스냅샷에서 버릴 것과 남길 것 {#2026-02-구현-스냅샷에서-버릴-것과-남길-것}
...
## 이미지 — 같은 샘 아래 두 개의 문 {#이미지-같은-샘-아래-두-개의-문}

{{< figure src="/images/20260730T200922--world-glgman-universe-pipeline__botlog_brand_worldbuilding.jpg" >}}

실제 독법: 뒤쪽 낡은 얼음-돌 오벨리스크(정본, 문자 없는 추상 물결무늬만 새겨짐)에서 앰버빛 물줄기 하나가 흘러나온다. 중앙에서 GLGMAN이 무기 아닌 드라이버로 낮은 돌 밸브를 조정해, 그 하나의 물줄기를 정확히 둘로 가른다. 왼쪽 문(적갈색 여우)은 두루마리 메뉴 앞에 서서 읽는 자세 — [이 노트]({{< relref "/notes/20260212T103100.md" >}})가 말하는 "인간은 인자 없이 실행해 메뉴를 읽는다"의 그림이다. 오른쪽 문(청회색 여우)은 토큰 모양 타일을 직접 짚는 자세 — "에이전트는 명시적 인자로 같은 기능을 호출한다"의 그림이다. 두 물줄기는 각자의 문을 지나 다시 굽이쳐 앞쪽 하나의 웅덩이로 합쳐지고, 그 색과 빛은 오벨리스크에서 처음 나온 물줄기와 동일하다 — "같은 기능"이라는 문장을 시각적 동일성으로 확인시킨다. 작은 펭귄 새끼가 웅덩이 곁에서 두 물줄기가 다시 합쳐지는 것을 지켜본다.

이 장면은 [@힣: Org SSOT 메타문서]({{< relref "/notes/20251021T105353.md" >}})의 문장 — "Org 정본과 CLI 실행면은 중복 문서가 아니라 한 파이프라인의 서로 다른 층이다" — 을 그대로 세계 이미지로 옮긴 것이다. 두 문서가 서로를 가리키던 상호링크가, 이번 회차로 하나의 장면 안에서도 만난다.
...
-   화면비: 16:9, 해상도: 2K
-   생성일: <span class="timestamp-wrapper"><span class="timestamp">[2026-07-30 Thu 20:10]</span></span> (오라클 서버, pi ACP Sonnet5)
...
## 원문 보존 — 2026-02-12 질문 {#원문-보존-2026-02-12-질문}
