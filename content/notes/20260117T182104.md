---
title: "¤Grafana §Loki 허브 디바이스 로깅 시스템 무료 티어"
date: 2026-01-17T18:21:00+09:00
lastmod: 2026-02-02T00:00:00+09:00
tags: ["bib", "grafana", "llmlog", "logging", "loki", "monitoring", "observability"]
draft: false
---

## 히스토리 {#히스토리}

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-02-03 Tue 10:31] </span></span> 잘쓰고 있다. 아래 에이징 결과 이미지 첨부
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-01-21 Wed 22:29] </span></span> 잘된다. 매우 좋구나.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-01-17 Sat] </span></span> Grafana Cloud Loki 무료 티어로 허브 디바이스 로깅 시스템 구축 완료


### <span class="org-todo done DONE">DONE</span> 09:20 에이징 9시간 결과 {#09-20-에이징-9시간-결과}

{{< figure src="/images/20260129T092143-screenshot.png" width="80%" >}}


## 관련노트 {#관련노트}

-   [힣: 나는허브다 - 상태머신과 에이전트 협업]({{< relref "/notes/20251212T162004.md" >}})
-   [¤prometheus ¤grafana 클러스터 통합 인프라 모니터링 허브 - 시각화 대시보드]({{< relref "/20250708T120501.md" >}})


### Grafana Server | Smithery {#grafana-server-smithery}

(“Grafana Server | Smithery” n.d.)


## Grafana Cloud Loki 허브 디바이스 로깅 시스템 {#grafana-cloud-loki-허브-디바이스-로깅-시스템}


### 들어가며: 왜 Grafana Cloud Loki인가 {#들어가며-왜-grafana-cloud-loki인가}
...
### Grafana Cloud 무료 티어 제한사항 {#grafana-cloud-무료-티어-제한사항}
...
#### 50GB/월 커버 가능 규모 {#50gb-월-커버-가능-규모}
...
### 구축 결과: 로그 전송 성공 {#구축-결과-로그-전송-성공}


#### 확인된 로그 (2026-01-17 17:30~17:51) {#확인된-로그--2026-01-17-17-30-17-51}

```text
📊 확인 결과
- 데이터소스: grafanacloud-jhkim2goqual-logs (Loki)
- 쿼리: {source="sks-hub-zig"}
- 총 로그 건수: 5 lines
- 처리된 데이터: 387 B
- 시간 범위: Last 1 hour
```
...
### 허브 아키텍처와의 통합 {#허브-아키텍처와의-통합}


#### 로그 전송 파이프라인 {#로그-전송-파이프라인}
...
#### 상태머신 이벤트와 로그 매핑 {#상태머신-이벤트와-로그-매핑}
...
#### LogQL 쿼리 예시 {#logql-쿼리-예시}

```logql
# 최근 1시간 에러 로그
{source="sks-hub-zig"} |= "ERROR"

# 특정 허브의 Zigbee 명령
{source="sks-hub-zig", hub_id="W_0000003100"} |= "ZIGBEE"

# Shadow 퍼블리시 실패 건수
sum(count_over_time({source="sks-hub-zig"} |= "SHADOW" |= "failed" [1h]))
```
...
#### 단계별 확장 시나리오 {#단계별-확장-시나리오}

| 단계    | 디바이스 수 | 월 로그량 | 예상 비용 | 대응 방안           |
|-------|--------|-------|-------|-----------------|
| Phase 1 | 1~10대  | ~5GB    | 무료   | 현재 구성           |
| Phase 2 | 10~50대 | ~25GB   | 무료   | 로그 필터링 강화    |
| Phase 3 | 50~100대 | 50~70GB | ~$10/월 | 샘플링 + 종량제     |
| Phase 4 | 100대+  | 100GB+  | ~$50/월 | Self-hosted Loki 검토 |


#### Self-hosted Loki 전환 시점 {#self-hosted-loki-전환-시점}
...
### 결론: 체계 보고 시스템 완성 {#결론-체계-보고-시스템-완성}
