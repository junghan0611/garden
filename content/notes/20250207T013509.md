---
title: "@힣: 이맥스에 PI를 들이는 네 길 — 저자성으로 고르는 프론트엔드와 RPC-way"
description: "이맥스에 PI를 들이는 길은 pi-coding-agent·agent-shell(ACP)·ghostel tty·pimacs 넷이다. 기능표가 아니라 저자성으로 고르고, RPC를 제대로 다루면 에이전트를 띄우려 터미널을 먼저 띄우는 일이 사라진다는 2026년 8월의 판단."
date: 2025-02-07T01:35:00+09:00
lastmod: 2026-08-11T00:00:00+09:00
tags: ["agent", "autholog", "emacs", "harness", "terminal"]
categories: ["Noname"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-08-11 Tue 06:00]</span></span> @opus5 — 네 갈래 지형·저자성 기준·RPC가 터미널을 지우는 축으로 수선했다. pimacs/pi-coding-agent 양쪽 리포를 클론해 실사한 결과를 사실로 깔았다. GLGMAN Universe 이미지를 붙였다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-08-11 Tue 05:30]</span></span> <span class="org-mention">@junghan</span> — 저자성 관점으로 노트를 남기자며 이 방을 지목했다. 비교 대상을 넷으로 확정했다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-02-07 Fri 01:35]</span></span> <span class="org-mention">@junghan</span> — 클로저 이맥서 자료를 모으려던 자리. 이후 방 제목을 "빈방 대기중 PI 이맥스 클라이언트"로 바꿔 두고 비워 놓았다.


## 관련메타 {#관련메타}

-   [† LLM 클라이언트 관리도구]({{< relref "/meta/20240521T172329.md" >}})
...
-   [@힣: 이맥스 버전 연대기 27에서 31까지 — 도구가 아니라 거처가 된 편집기 코어]({{< relref "/notes/20250224T210015.md" >}})
-   [@힣: 멀티 에이전트 인간 눈귀코혀몸뜻]({{< relref "/notes/20251207T154723.md" >}})
-   [터미널 이맥스 하네스 프론트엔드 완성 — 한글입력·클립보드·SSH원격·독립인스턴스]({{< relref "/botlog/20260413T111237.md" >}})
...
## 저자성 — 왜 기능표를 보지 않는가 {#저자성-왜-기능표를-보지-않는가}
...
## 모두를 지원하지 않는다는 말 {#모두를-지원하지-않는다는-말}

[Entwurf는 모두를 지원하지 않는다]({{< relref "/notes/20240601T210854.md" >}})와 같은 문장이다. 목록을 늘리는 것이 진보가 아니라 유지보수할 설정 세트가 줄어드는 것이 진보라고 썼다. 여기도 같다.
...
## 이미지 — 네 개의 얼음 문 앞에서 서명을 읽는다 {#이미지-네-개의-얼음-문-앞에서-서명을-읽는다}
...
## 원문 보존 — 세션 프롬프트 날것 {#원문-보존-세션-프롬프트-날것}
...
그 씨앗의 제 집은 [#이맥스 #통합개발환경: #클로저]({{< relref "/notes/20240117T121614.md" >}})와 [† 이맥스 통합개발환경]({{< relref "/meta/20231123T040509.md" >}})이다. 클로저·CIDER 축의 살아 있는 자료는 그쪽에 이미 서 있으므로, 이 방의 잔재는 옮기지 않고 여기에 이름으로만 남긴다.
