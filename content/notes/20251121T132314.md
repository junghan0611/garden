---
title: "Emacs-AI 통합 패러다임: MCP, REPL, 그리고 진정한 협업"
date: 2025-11-21T13:23:00+09:00
lastmod: 2025-12-07T22:01:09+09:00
tags: ["llmlog", "emacs", "mcp", "repl", "agent-shell", "opencode", "elisp"]
draft: false
---
...
## Emacs-AI 통합 패러다임 {#emacs-ai-통합-패러다임}
...
### 세 가지 통합 패러다임 {#세-가지-통합-패러다임}


#### 1. 네이티브 Elisp Eval 방식 (직접 통합) {#1-dot-네이티브-elisp-eval-방식--직접-통합}


##### <span class="org-todo todo TODO">TODO</span> Efrit (Steve Yegge) {#efrit--steve-yegge}
...
##### DONT Emigo {#dont-emigo}


#### 2. MCP 기반 방식 (표준 프로토콜) {#2-dot-mcp-기반-방식--표준-프로토콜}
...
##### A. 이맥스를 MCP 서버로 만들기 {#a-dot-이맥스를-mcp-서버로-만들기}
...
##### B. 이맥스 내에서 MCP 클라이언트 사용 {#b-dot-이맥스-내에서-mcp-클라이언트-사용}
...
#### 3. ACP 기반 방식 (Agent Client Protocol) {#3-dot-acp-기반-방식--agent-client-protocol}
...
#### 지금 당장 시도할 만한 것 {#지금-당장-시도할-만한-것}


##### 1. gptel + mcp.el (추천 ⭐⭐⭐⭐⭐) {#1-dot-gptel-plus-mcp-dot-el--추천}
...
##### 2. elisp-dev-mcp (추천 ⭐⭐⭐⭐) {#2-dot-elisp-dev-mcp--추천}
...
##### 3. emacs-mcp-server (주의해서 ⚠️) {#3-dot-emacs-mcp-server--주의해서-️}
...
##### Phase 1: MCP 클라이언트 (Emacs 내부) {#phase-1-mcp-클라이언트--emacs-내부}
...
##### Phase 2: MCP 서버 (Emacs를 노출) {#phase-2-mcp-서버--emacs를-노출}
...
##### Phase 3: 양방향 통합 (REPL 방식) {#phase-3-양방향-통합--repl-방식}
...
#### 1. Elisp 개발 워크플로우 {#1-dot-elisp-개발-워크플로우}
...
#### 3. Org-mode 워크플로우 {#3-dot-org-mode-워크플로우}
...
#### 2. MCP 서버 선택적 활성화 {#2-dot-mcp-서버-선택적-활성화}
...
#### 2. agent-shell 연동 확인 {#2-dot-agent-shell-연동-확인}
...
#### Phase 4: 고급 (진행 중) {#phase-4-고급--진행-중}
...
-   [agent-shell + OpenCode 커스텀 가이드]({{< relref "20251121T131412--agent-shell-opencode-커스텀-가이드__llmlog_agent_shell_opencode_acp_emacs" >}})
-   [브라우저 자동화 도구 비교]({{< relref "20251121T122829--브라우저-자동화-도구-비교__llmlog_playwright_kapture_browserbase_mcp" >}})


### APPENDIX: OpenCode agent-shell-mode 구현 제안 {#appendix-opencode-agent-shell-mode-구현-제안}
...
##### OpenCode 설정 업데이트 {#opencode-설정-업데이트}
...
##### 4. Org-mode 작업 {#4-dot-org-mode-작업}
