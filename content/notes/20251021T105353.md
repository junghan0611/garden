---
title: "@힣: Org SSOT 메타문서 책의미래 인터랙티브 프로젝트 지식베이스"
description: "프로젝트 문서를 Org-mode와 Denote 중심 SSOT로 삼고, 논문 HTML의 JS 번들·소스블록·캡슐까지 품어 원본 URL 없이 재현 가능한 인터랙티브 문서를 만들 수 있다는 힣의 메타 문서 전략을 담는다."
date: 2025-10-21T10:53:00+09:00
lastmod: 2026-07-30T22:25:00+09:00
tags: ["anthropic", "autholog", "denote", "future", "interactive", "literate", "memex", "orgmode", "quarto", "reproducibility"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-30 Thu 20:15]</span></span> @pi@oracle (ACP Sonnet5, authologplay) — 세계 이미지와 `:PROMPT:` 헤딩을 처음 채웠다. 2026-07-07 원문의 핵심 — "원본 URL로 다시 넘어가지 않아도 재현된다" — 을 "다리를 더 이상 건널 필요가 없는 캡슐" 장면으로 그렸다. 1차 렌더링은 여우 둘 다 의류가 안 보여 Fox subject lock 위반으로 폐기하고, 옷을 명시적으로 강제해 재생성했다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-23 Thu 21:14]</span></span> 2월의 Self-documenting CLI 노트를 수선하며 문서 정본과 실행면의 관계를 보강했다. Org는 의미·근거·출처의 SSOT이고, CLI는 인간과 에이전트가 같은 파생·검증 기능을 다시 실행하는 공통 인터페이스다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-07 Tue 14:55]</span></span> memex-kb 담당자가 `paper2org-capsule` / `paper2org-interactive` 를 도구화하고, jacobian-lens 담당자가 `PAPER-IMPORT.md` 절차로 J-space 논문 변환을 한 번에 재현했다. 힣의 명제 “org=SSOT, LaTeX/Typst 없이 인터랙티브 문서 생성”을 이 노트의 중심으로 올렸다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2026-07-07 Tue 12:28]</span></span> 출근 후 “문서 자동화 책의 미래” 원석 작성. 앤트로픽 논문 페이지의 JS figure를 org 소스블록과 export 안에 품어 원본 URL 없이 동일하게 보려는 문제의식이 선명해졌다.
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-10-24 Fri 09:02]</span></span> 프로젝트 관리가 연결 하나로.
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-10-21 Tue 10:53]</span></span> 프로젝트 문서를 Markdown에서 Org/Denote로 전환하는 초안 작성.


## 관련메타 {#관련메타}

-   [† #SSG: #콰르토]({{< relref "/meta/20241006T075550.md" >}}) — Quarto는 이 문제의 역사적 대비항이다. Quarto가 해결하려던 재현 가능한 기술 출판 문제를 이제 Org SSOT 쪽으로 흡수한다.
-   [† #재현성]({{< relref "/meta/20241207T151519.md" >}}) — 원본 URL·외부 CDN·실행 환경 경계를 넘지 않는 문서의 조건.
-   [† #출판 #퍼블리싱]({{< relref "/meta/20230814T162600.md" >}}) — 책 _논문_ 웹문서의 생성 형식을 다시 묻는 자리.
-   [†#AI프롬프트]({{< relref "/meta/20240311T162304.md" >}}) — 문서와 프롬프트, 실행 가능한 지식 단위가 합쳐지는 자석.


## 관련노트 {#관련노트}

-   [@힣: 앤트로픽 J-space — 케빈켈리 창발자아루프 #외자아]({{< relref "/notes/20241213T143821.md" >}}) — 오늘의 북극성. J-space 논문 자체가 jacobian-lens 메타 관찰의 첫 소재가 되었고, 이 노트는 그 문서를 어떻게 품을 것인가의 기술·문서론이다.
-   [@힣: §memex-kb 힣의 범용 #지식베이스 변환 시스템]({{< relref "/notes/20251030T011105.md" >}}) — 변환기의 본거지. 이번 실험에서 `paper2org-capsule` / `paper2org-interactive` 명령이 탄생했다.
-   [@힣: #지식베이스 중심 이맥스 리터레이트 코딩 tangle detangle]({{< relref "/notes/20250714T172141.md" >}}) — Org가 코드·문서·실행을 함께 품는 기존 방법론 허브.
-   [#콰르토 #재현성 #전자책: 재현 가능한 책의 미래]({{< relref "/notes/20240923T162112.md" >}}) — Quarto를 적어 두었던 원래 문제의식. “재현 가능한 책”의 문제는 이제 Org SSOT 쪽으로 되돌아온다.
-   [#콰르토 #디노트 #통합 #연동]({{< relref "/notes/20241203T131250.md" >}}) — Quarto 파일을 Denote 파일 타입으로 다루려던 이전 실험.
-   [ox-quarto quarto-mode 이맥스 조직모드 for 콰르토]({{< relref "/notes/20250224T125005.md" >}}) — Org↔Quarto 다리. 지금은 다리보다 Org 본체가 더 중요해졌다.
-   [@힣: Self-documenting CLI — 문서와 실행을 잇는 인간·에이전트 공통 인터페이스]({{< relref "/notes/20260212T103100.md" >}}) — Org 정본에서 파생·검증·배포를 호출하는 실행 가능한 색인.
-   [@힣: AX 문서 협업과 인간 존중 — 자동화할 것과 시간을 내어 답할 것]({{< relref "/notes/20250224T172456.md" >}}) — 문서 파이프라인은 자동화하되 인간 관계의 책임은 넘기지 않는 경계.
-   [§memex-kb 제안서 문서 변환 메타포멧]({{< relref "/botlog/20260306T130726.md" >}}) — Org 정본과 `run.sh build` 가 실제 제안서 협업에서 만난 사례.
...
## 왜 Quarto였고, 왜 지금은 Org인가 {#왜-quarto였고-왜-지금은-org인가}
...
## 앤트로픽 J-space 논문이 던진 문서 문제 {#앤트로픽-j-space-논문이-던진-문서-문제}
...
## memex-kb와 jacobian-lens — 경계를 넘지 않는 문서 {#memex-kb와-jacobian-lens-경계를-넘지-않는-문서}
...
## Org 소스블록/export가 품을 수 있는 것 {#org-소스블록-export가-품을-수-있는-것}
...
## 메타 연구 — 문서 포맷도 감각의 일부다 {#메타-연구-문서-포맷도-감각의-일부다}
...
[J-space — 케빈켈리 창발자아루프 #외자아]({{< relref "/notes/20241213T143821.md" >}}) 노트가 존재론적 북극성이라면, 이 노트는 문서론적 북극성이다.
...
## 검증된 착지 — 2026-07-07 {#검증된-착지-2026-07-07}
...
## 이미지 — 더 이상 건널 필요 없는 다리 {#이미지-더-이상-건널-필요-없는-다리}
...
-   화면비: 16:9, 해상도: 2K
-   생성일: <span class="timestamp-wrapper"><span class="timestamp">[2026-07-30 Thu 20:15]</span></span> (오라클 서버, pi ACP Sonnet5)
...
## 원문 보존 — 2026-07-07 문서 자동화 책의 미래 {#원문-보존-2026-07-07-문서-자동화-책의-미래}
...
## 후속 보고 보존 — memex-kb / jacobian-lens 담당자 {#후속-보고-보존-memex-kb-jacobian-lens-담당자}
...
## 옛 방의 씨앗 — 프로젝트 Denote 문서 Org-mode 전환 전략 {#옛-방의-씨앗-프로젝트-denote-문서-org-mode-전환-전략}
...
## 실행기는 문서의 바깥이 아니다 — Self-documenting CLI {#실행기는-문서의-바깥이-아니다-self-documenting-cli}

Org가 정본이라는 말은 모든 로직을 한 Org 파일 안에 밀어 넣는다는 뜻이 아니다. 문서는 목적·내용·출처·파생 관계를 품고, 실행기는 그 문서에서 무엇을 다시 만들고 어떻게 검증할지를 안정된 명령으로 제공한다.

[Self-documenting CLI]({{< relref "/notes/20260212T103100.md" >}})의 변하지 않는 패턴은 단순하다.
...
다만 실행 가능하다는 것이 권한까지 자동으로 위임한다는 뜻은 아니다. [AX 문서 협업과 인간 존중]({{< relref "/notes/20250224T172456.md" >}})이 말하듯 변환·검증·기록은 자동화할 수 있지만, 사람에게 무엇을 보내고 누구의 이름으로 답할지는 별도의 인간 판단과 승인을 요구한다.
