...
title: "#조직모드 #가이드 #튜토리얼"
author: ["Junghan Kim"]
date: 2024-12-15T12:04:00+09:00
lastmod: 2024-12-15T12:07:00+09:00
tags: ["guide", "notes", "orgmode", "tutorial"]
categories: ["Noname"]
draft: false
---
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2025-06-08 Sun 11:37] </span></span> 양식 검토만
-   [2024-12-15]({{< relref "/journal/20241215T000000.md" >}}) 가이드 매뉴얼 문서가 없더라. 일단 판다. 작성할 필요가? 없다. 줍줍해야지. 자료 만드는게 일이 되면 안된다.


## 유튜브에서 배움 아래 정보 확인 바람 {#유튜브에서-배움-아래-정보-확인-바람}

<span class="timestamp-wrapper"><span class="timestamp">[2022-11-18 Fri 14:57]</span></span>
...
5.  Drawers, Logging/quick notes :: 매우 유용
    ...
    -   `C-c C-z` take quick notes. 로그북으로 캡처가 된다. 괜찮아! LOGBOOK:
        ...
        END:
6.  Acrhiving :: 중요한 주제다
    -   Finished tasks clutter up your org file, so its time to move them out of the way. Define a global archive file with <kbd>#+ARCHIVE: filename.org::</kbd>
...
7.  Automatic logging of status changes - 이거 신기하다!
    ...
    -   완료 상태가 되면, 타임스탬프에 CLOSED 라고 로그를 남기고 싶다면?! customize variable :: <kbd>org-log-done</kbd>
    -   `스케줄을 바꾸었을 때` 타임스탬프에 로그를 남겨 놓고 싶다면?! customize variable :: <kbd>org-log-reschedule</kbd> 이건 매우 필요하다.
...
## <span class="org-todo todo TODO">TODO</span> 대기중 리스트 {#대기중-리스트}

<span class="timestamp-wrapper"><span class="timestamp">[2024-12-07 Sat 15:37]</span></span>


### GTD and Deepwork Workflow {#gtd-and-deepwork-workflow}

<span class="timestamp-wrapper"><span class="timestamp">[2022-11-24 Thu 13:57]</span></span>
...
### 파일 링크 작성 방법 : Org Internal Links {#h:a64c7c97-623b-406c-8ade-711c2596b187}

<span class="timestamp-wrapper"><span class="timestamp">[2022-11-29 Tue 05:34]</span></span>

-   헤딩을 만드는 양식은 다양하다
-   255 / My Target / \*My Target / #my-custom-id / _regexp_ 가 있다.
-   일단 파일 경로를 복사하고, 스니펫으로 양식을 불러온 뒤에, 입력하면 된다.

-   [Org Internal Links | Irreal - irreal.org](https://irreal.org/blog/?p=12151)


### Creating Org Heading Tables {#creating-org-heading-tables}

:CUSTOM_ID: 495202e5-43ca-4c66-9513-67c6176780ef

<span class="timestamp-wrapper"><span class="timestamp">[2023-10-13 Fri 11:03]</span></span>

신기하다. 괜찮다.


#### Org full-text search {#org-full-text-search}
...
```
<div class="src-block-caption">
  <span class="src-block-number"><a href="#code-snippet--install-helm-rg">Code Snippet 2</a>:</span>
  Install helm-rg
</div>
...
```
<div class="src-block-caption">
  <span class="src-block-number"><a href="#code-snippet--install-org-rifle">Code Snippet 3</a>:</span>
  Install org-rifle
</div>
...
-   [조직모드: 이맥스 파이썬 문서 코딩 도구 - 콰르토 주피터 호환 - 나우]({{< relref "/notes/20240603T174450.md" >}})


### 도구들 {#도구들}


### 생산성 및 지식 관리 {#생산성-및-지식-관리}

-   [조직모드: 세컨드 브레인 제텔카스텐 파라]({{< relref "/notes/20231005T135300.md" >}})


#### 어젠다 최적화 이슈 {#어젠다-최적화-이슈}

둠에 다 반영되어 있다. 참고하자. 헤딩이 왜 다 펼쳐져 있는지에 대한 답이 되었다.


##### Reduce the number of Org agenda files {#reduce-the-number-of-org-agenda-files}
...
##### Inhibit agenda files startup options (Org &gt; 8.0) {#inhibit-agenda-files-startup-options--org-8-dot-0}
...
특히 Org는 [BROKEN LINK: doc::org-startup-folded] 또는 `#+STARTUP: folded` 에 설정된 대로 스타트업 공개 상태를 준수합니다.
...
## INFO 매뉴얼 목차 {#info-매뉴얼-목차}

-   [#활용법: #이맥스 #인포 #매뉴얼]({{< relref "/notes/20221004T150100.md" >}}) 일부 이다.
