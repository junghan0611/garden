...
### 🎯 핵심 발견: Claude Web의 강력한 기능 {#핵심-발견-claude-web의-강력한-기능}
...
### 📊 meta-config 최신 현황 (2025-11-11) {#meta-config-최신-현황--2025-11-11}
...
#### 최근 주요 커밋 (Nov 11) {#최근-주요-커밋--nov-11}
...
### 🏗️ 6-Layer 아키텍처 (최신) {#️-6-layer-아키텍처--최신}
...
### 🆕 memacs Layer (핵심 혁신) {#memacs-layer--핵심-혁신}
...
### 🔄 3-Protocol 체계 {#3-protocol-체계}


#### 1. ACP (Agent Client Protocol) {#1-dot-acp--agent-client-protocol}

**User ↔ Meta Agent** - Emacs agent-shell 통합 - 디바이스 컨텍스트 인식 - 토큰 사용량 관리


#### 2. A2A (Agent-to-Agent) {#2-dot-a2a--agent-to-agent}

**Meta Agent ↔ Domain Agents** - JSON-RPC 2.0 기반 - 컨텍스트 격리 - 명시적 위임 프로토콜


#### 3. MCP (Model Context Protocol) {#3-dot-mcp--model-context-protocol}
...
#### "Agents All the Way Down" {#agents-all-the-way-down}
...
### 🛠️ 개발 가이드라인 (NixOS 특화) {#️-개발-가이드라인--nixos-특화}


#### 1. Python 프로젝트 {#1-dot-python-프로젝트}
...
#### Phase 1: Q4 2025 (진행 중) {#phase-1-q4-2025--진행-중}
...
#### Phase 2: Q1 2026 {#phase-2-q1-2026}
...
#### Phase 3: Q2 2026 {#phase-3-q2-2026}
...
#### Phase 4: Q3+ 2026 {#phase-4-q3-plus-2026}
...
### 🔗 claude-config 연관성 {#claude-config-연관성}


#### meta-config vs claude-config {#meta-config-vs-claude-config}
...
### 💡 claude-config에 적용할 개념 {#claude-config에-적용할-개념}


#### 1. memacs Layer 도입 {#1-dot-memacs-layer-도입}
...
#### 2. Context Isolation {#2-dot-context-isolation}
...
### ✅ 즉시 적용 가능한 것 {#즉시-적용-가능한-것}


#### 1. CLAUDE.md 개선 {#1-dot-claude-dot-md-개선}

meta-config의 CLAUDE.md 패턴 적용: - 환경 설정 가이드라인 - NixOS 제약사항 - 디바이스 컨텍스트 - 프로젝트별 규칙


#### 2. NixOS 가이드라인 강화 {#2-dot-nixos-가이드라인-강화}
...
### 🎯 제안: claude-config 업데이트 {#제안-claude-config-업데이트}


#### Option 1: CLAUDE.md 개선 (즉시) {#option-1-claude-dot-md-개선--즉시}

meta-config의 개발 가이드라인 추가: - Python: shell.nix - Bash: 명령어 치환 제약 - Denote 문서화


#### Option 2: 아키텍처 문서 추가 (단기) {#option-2-아키텍처-문서-추가--단기}
...
#### Option 3: 리팩토링 (장기) {#option-3-리팩토링--장기}
...
### 🎯 핵심 발견: 모든 퍼즐의 마지막 조각 {#핵심-발견-모든-퍼즐의-마지막-조각}
...
### 📊 Google Workspace MCP 서버란? {#google-workspace-mcp-서버란}
...
### 🚀 힣's 유스케이스: 완벽한 매칭 {#힣-s-유스케이스-완벽한-매칭}


#### 1. PM 에이전트 - 캘린더 통합 (🔥 최우선) {#1-dot-pm-에이전트-캘린더-통합--최우선}
...
#### 3. 협업 문서화 &amp; 채팅 {#3-dot-협업-문서화-and-채팅}
...
#### Step 1: Google Cloud 프로젝트 설정 {#step-1-google-cloud-프로젝트-설정}
...
#### Step 2: MCP 서버 설치 {#step-2-mcp-서버-설치}
...
#### Step 3: Claude Code 통합 {#step-3-claude-code-통합}
...
#### 시나리오 1: PM 에이전트 - 주간 리포트 자동 생성 {#시나리오-1-pm-에이전트-주간-리포트-자동-생성}
...
#### 시나리오 2: 여행 에이전트 - 하와이 여행 자동 관리 {#시나리오-2-여행-에이전트-하와이-여행-자동-관리}
...
#### 시나리오 3: 메모리 시스템 ↔ Drive 동기화 {#시나리오-3-메모리-시스템-drive-동기화}
...
### 🎯 힣's 우선순위 적용 {#힣-s-우선순위-적용}


#### Phase 1: PM 에이전트 캘린더 통합 (이번주) {#phase-1-pm-에이전트-캘린더-통합--이번주}
...
#### Phase 2: 여행 에이전트 (1-2주) {#phase-2-여행-에이전트--1-2주}
...
#### Phase 3: 전체 통합 (1개월) {#phase-3-전체-통합--1개월}
...
#### Google Workspace MCP 추가 {#google-workspace-mcp-추가}
...
#### After (Google Workspace MCP) {#after--google-workspace-mcp}
...
#### Phase 4: meta-config 통합 (장기) {#phase-4-meta-config-통합--장기}
...
#### memacs Layer 연동 {#memacs-layer-연동}
...
#### 이번주 (2025-11-12 ~ 11-15) {#이번주--2025-11-12-11-15}
...
#### 다음 주 (11-18 ~ 11-22) {#다음-주--11-18-11-22}
...
##### 1. 리포 → 인간 포맷 (자동 변환) {#1-dot-리포-인간-포맷--자동-변환}
...
##### 2. 인간 입력 → 리포 반영 (양방향 동기화) {#2-dot-인간-입력-리포-반영--양방향-동기화}
...
### 🔧 Google Workspace MCP 역할 {#google-workspace-mcp-역할}


#### MCP 서버 = 변환 레이어의 핵심 {#mcp-서버-변환-레이어의-핵심}
...
#### Pattern 1: PM 에이전트 (시간 관리) {#pattern-1-pm-에이전트--시간-관리}
...
#### Pattern 2: 여행 에이전트 (자동 일정 관리) {#pattern-2-여행-에이전트--자동-일정-관리}
...
#### Pattern 3: 채팅 자동 응답 {#pattern-3-채팅-자동-응답}
...
#### Pattern 4: 인간 협업 위임 (AI 한계 극복) {#pattern-4-인간-협업-위임--ai-한계-극복}
...
#### 1. 브라우저 없는 완전 자동화 {#1-dot-브라우저-없는-완전-자동화}
...
#### 2. 리포 = 단일 진실 원천 {#2-dot-리포-단일-진실-원천}
...
#### 사례 1: SKS Gateway TC 마감 관리 {#사례-1-sks-gateway-tc-마감-관리}
...
#### 사례 2: Anthropic 지원서 준비 {#사례-2-anthropic-지원서-준비}
...
#### Phase 1: Google Workspace MCP 설정 (1주) {#phase-1-google-workspace-mcp-설정--1주}
...
#### Phase 2: PM 에이전트 통합 (2주) {#phase-2-pm-에이전트-통합--2주}
...
#### Phase 3: 여행 에이전트 (2주) {#phase-3-여행-에이전트--2주}
...
#### Phase 4: 채팅 자동 응답 (3주) {#phase-4-채팅-자동-응답--3주}
...
#### meta-config 6-Layer와의 관계 {#meta-config-6-layer와의-관계}
...
#### 1. "실제 창조물은 리포에" {#1-dot-실제-창조물은-리포에}
...
#### 2. "인간이 원하는 포맷으로 변환" {#2-dot-인간이-원하는-포맷으로-변환}
...
#### 3. "레거시 도구 = 로그/버그 데이터" {#3-dot-레거시-도구-로그-버그-데이터}
...
### 핵심 가치: 에이전트-인간 협업 최적화 {#핵심-가치-에이전트-인간-협업-최적화}
...
#### Personal/Work 완전 분리 {#personal-work-완전-분리}
...
#### MCP 서버 설정 (`~/.mcp.json`) {#mcp-서버-설정--dot-mcp-dot-json}
...
#### Wrapper 스크립트 구조 {#wrapper-스크립트-구조}
...
### OAuth 인증 메커니즘 {#oauth-인증-메커니즘}


#### 로컬 디바이스 (브라우저 직접 접근) {#로컬-디바이스--브라우저-직접-접근}
...
#### 원격 디바이스 (SSH 포트 포워딩) {#원격-디바이스--ssh-포트-포워딩}
...
### 멀티 디바이스 OAuth 인증 프로세스 {#멀티-디바이스-oauth-인증-프로세스}


#### 브라우저 인증 vs 텍스트 링크 방식 {#브라우저-인증-vs-텍스트-링크-방식}
...
### MCP 서버 리소스 관리 {#mcp-서버-리소스-관리}


#### 현재 구조의 문제점 {#현재-구조의-문제점}
...
#### 시나리오 1: 개인 Gmail 확인 (로컬) {#시나리오-1-개인-gmail-확인--로컬}
...
#### 시나리오 2: 업무 캘린더 확인 (원격) {#시나리오-2-업무-캘린더-확인--원격}
...
#### 시나리오 3: Personal/Work 동시 사용 {#시나리오-3-personal-work-동시-사용}
...
### SSH Config 자동화 {#ssh-config-자동화}


#### 포트 포워딩 자동 설정 {#포트-포워딩-자동-설정}
...
#### Personal/Work 데이터 혼재 {#personal-work-데이터-혼재}
...
#### OAuth 인증 후에도 계속 요청 {#oauth-인증-후에도-계속-요청}
...
#### 2단계: 지속적 MCP 인스턴스 {#2단계-지속적-mcp-인스턴스}
...
### 🎯 핵심 발견: 토큰 효율적 에이전트 확장 {#핵심-발견-토큰-효율적-에이전트-확장}
...
#### 해결책: Apps Script 내부 처리 {#해결책-apps-script-내부-처리}
...
### 🚀 2025-11-14 통합 완료 {#2025-11-14-통합-완료}
...
### 🎨 Apps Script의 강력한 확장성 {#apps-script의-강력한-확장성}


#### IDE 통합과 같은 개념 {#ide-통합과-같은-개념}
...
#### Apps Script가 제공하는 것 {#apps-script가-제공하는-것}
...
### 💡 Apps Script vs GCP: 명확한 차이 {#apps-script-vs-gcp-명확한-차이}
...
### 🎯 실전 활용: 토큰 효율화 시나리오 {#실전-활용-토큰-효율화-시나리오}


#### 시나리오 1: 주간 프로젝트 리포트 {#시나리오-1-주간-프로젝트-리포트}
...
#### 시나리오 2: PM 에이전트 - 할일 자동 생성 {#시나리오-2-pm-에이전트-할일-자동-생성}
...
#### 시나리오 3: 협업 - 팀 리포트 자동 생성 {#시나리오-3-협업-팀-리포트-자동-생성}
...
### 🏗️ 아키텍처 확장: Apps Script Layer {#️-아키텍처-확장-apps-script-layer}


#### 기존 6-Layer + Apps Script (7-Layer) {#기존-6-layer-plus-apps-script--7-layer}
...
#### Apps Script Layer의 역할 {#apps-script-layer의-역할}
...
### 💼 협업 관점: PM 에이전트의 날개 {#협업-관점-pm-에이전트의-날개}


#### Google Tasks + Sheets + Chat 통합 {#google-tasks-plus-sheets-plus-chat-통합}
...
#### 협업 시나리오: 문서 자동 생성 {#협업-시나리오-문서-자동-생성}
...
#### Phase 1: 기본 자동화 (현재 완료) {#phase-1-기본-자동화--현재-완료}
...
#### Phase 2: 토큰 효율화 (1-2주) {#phase-2-토큰-효율화--1-2주}
...
#### Phase 3: 완전 자동화 (1개월) {#phase-3-완전-자동화--1개월}
...
#### Phase 4: AI 통합 (2-3개월) {#phase-4-ai-통합--2-3개월}
...
### 🔗 Apps Script 리소스 {#apps-script-리소스}
