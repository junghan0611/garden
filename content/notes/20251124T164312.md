---
title: "Beads Agent Mail 멀티에이전트 병렬작업 도입"
description: "Beads에 Agent Mail 개념을 도입해 멀티에이전트 병렬 작업을 검토한 노트이다. 실제 활용 흐름을 담았다."
date: 2025-11-24T16:43:00+09:00
lastmod: 2025-11-24T16:43:00+09:00
tags: ["agentmail", "beads", "claudecode", "llmlog", "multiagent", "opencode"]
draft: false
---

> [!abstract] 이 노트에 대하여
> 
> 여러 AI 에이전트를 병렬로 돌릴 때 Git-only 방식보다 Agent Mail이 어떤 장점을 갖는지 비교한다. 멀티에이전트 협업의 지연과 충돌 문제를 구조적으로 다루는 노트다.


## Beads Agent Mail 멀티에이전트 병렬작업 도입 {#beads-agent-mail-멀티에이전트-병렬작업-도입}


### 배경 {#배경}

로컬 환경에서 Claude Code, OpenCode 등 여러 AI 에이전트를 병렬로 실행하여 작업 효율을 높이기 위한 Agent Mail 도입 검토.


#### 관련 문서 {#관련-문서}

-   [beads-macro-memory-system-설계-분석]({{< relref "/notes/20251123T191835.md" >}})
-   [힣-에이전트-생존-그리고-마이크로뷰-매크로뷰]({{< relref "/notes/20251122T210521.md" >}})
-   [클로드코드-오픈코드-에이전트-통합-구성]({{< relref "/notes/20251121T204537.md" >}})


### 문제 정의 {#문제-정의}


#### Git-only 모드의 한계 {#git-only-모드의-한계}

| 항목  | Git-only        | Agent Mail      |
|-----|-----------------|-----------------|
| 지연시간 | 2-5초           | &lt;100ms       |
| 충돌 감지 | 사후 (git pull 후) | 즉시 (HTTP 409) |
| 작업 예약 | 없음            | Reservation TTL |


#### 발생 가능한 시나리오 {#발생-가능한-시나리오}

```text
Agent A: bd update sks-123 --status in_progress
   ↓ (30초 debounce)
   ↓ export to JSONL → git commit/push

Agent B: bd update sks-123 --status in_progress
   ↓ (동시 작업 시작 - 충돌!)
```


### 해결책: Agent Mail {#해결책-agent-mail}


#### 아키텍처 {#아키텍처}

```text
┌─────────────────────────────────────────────┐
│ bd (Beads CLI)                              │
│                                             │
│  ┌─────────────┐      ┌─────────────────┐  │
│  │ Git Sync    │      │ Agent Mail      │  │
│  │ (필수)      │      │ (선택-권장)     │  │
│  └─────────────┘      └─────────────────┘  │
│         │                      │            │
└─────────┼──────────────────────┼────────────┘
          │                      │
          ▼                      ▼
  ┌──────────────┐      ┌──────────────┐
  │ .beads/      │      │ Agent Mail   │
  │ issues.jsonl │      │ Server       │
  │ (git)        │      │ (HTTP)       │
  └──────────────┘      └──────────────┘
```


#### 보호 메커니즘 계층 {#보호-메커니즘-계층}

| 계층 | 메커니즘               | 보호 대상 | 지연시간  |
|----|--------------------|-------|-------|
| 1  | Agent Mail Reservation | 이슈 충돌 방지 | &lt;100ms |
| 2  | Exclusive Lock         | 데이터베이스 전체 | 즉시      |
| 3  | Git Sync               | 데이터 영속성 | 2-5초     |
| 4  | Daemon 격리            | 프로젝트 격리 | -         |


### 설치 및 설정 {#설치-및-설정}


#### 1. Agent Mail 서버 설치 {#1-dot-agent-mail-서버-설치}

```bash
cd ~/repos/3rd
git clone https://github.com/Dicklesworthstone/mcp_agent_mail.git
cd mcp_agent_mail
python3 -m venv .venv
source .venv/bin/activate
pip install -e .
```


#### 2. 서버 실행 {#2-dot-서버-실행}

```bash
# 별도 터미널에서 실행
cd ~/repos/3rd/mcp_agent_mail
source .venv/bin/activate
python -m mcp_agent_mail.cli serve-http
# http://127.0.0.1:8765 에서 실행
```


#### 3. 각 터미널 환경 변수 설정 {#3-dot-각-터미널-환경-변수-설정}

```bash
# 터미널 1 (Claude Code - Agent Alpha)
export BEADS_AGENT_MAIL_URL=http://127.0.0.1:8765
export BEADS_AGENT_NAME=claude-alpha
export BEADS_PROJECT_ID=sks-gateway

# 터미널 2 (OpenCode - Agent Beta)
export BEADS_AGENT_MAIL_URL=http://127.0.0.1:8765
export BEADS_AGENT_NAME=opencode-beta
export BEADS_PROJECT_ID=sks-gateway  # 동일해야 충돌 감지됨

# 터미널 3 (Claude Code - Agent Gamma)
export BEADS_AGENT_MAIL_URL=http://127.0.0.1:8765
export BEADS_AGENT_NAME=claude-gamma
export BEADS_PROJECT_ID=sks-gateway
```


### 워크플로우 {#워크플로우}


#### 병렬 작업 시작 {#병렬-작업-시작}

```bash
# 1. 작업 시작 전 - 가용 이슈 확인
cd ~/repos/work/sks-gateway-forked
bd ready --json

# 2. 이슈 claim (먼저 claim한 에이전트가 소유)
bd update sks-xxx --status in_progress
# → Agent Mail이 자동으로 reservation 생성
# → 다른 에이전트가 같은 이슈 claim 시 HTTP 409 에러

# 3. 작업 수행...

# 4. 완료 시
bd close sks-xxx --reason "구현 완료"
# → Reservation 자동 해제

# 5. 세션 종료 시 (필수!)
bd sync
git push
```


#### 충돌 발생 시 {#충돌-발생-시}

```bash
# Agent B가 이미 예약된 이슈 claim 시도
bd update sks-123 --status in_progress
# Error: sks-123 already reserved by claude-alpha

# 해결: 다른 이슈 선택
bd ready --json
bd update sks-456 --status in_progress
```


### Graceful Degradation {#graceful-degradation}

Agent Mail 서버가 다운되어도 beads는 정상 동작:

```bash
# 서버 다운 시 경고 메시지
WARN Agent Mail unavailable, falling back to git-only mode

# 모든 명령어 정상 동작 (Git-only 모드)
bd ready
bd update sks-xxx --status in_progress
```


### 운영 팁 {#운영-팁}


#### 서버 상태 확인 {#서버-상태-확인}

```bash
# 서버 health 확인
curl http://127.0.0.1:8765/api/health

# 현재 reservations 확인
curl http://127.0.0.1:8765/api/reservations

# Web UI
open http://127.0.0.1:8765/mail
```


#### Stale Reservation 정리 {#stale-reservation-정리}

```bash
# 에이전트 crash 후 reservation 수동 해제
curl -X DELETE http://127.0.0.1:8765/api/reservations/sks-123

# 또는 서버 재시작 (모든 ephemeral 상태 초기화)
pkill -f "mcp_agent_mail"
python -m mcp_agent_mail.cli serve-http
```


### 프로젝트 AGENTS.md 설정 {#프로젝트-agents-dot-md-설정}

```markdown
## Agent Mail Configuration

이 프로젝트는 멀티에이전트 병렬 작업을 위해 Agent Mail을 사용합니다.

### 환경 변수 설정 (필수)

\`\`\`bash
export BEADS_AGENT_MAIL_URL=http://127.0.0.1:8765
export BEADS_AGENT_NAME=<unique-agent-name>
export BEADS_PROJECT_ID=sks-gateway
\`\`\`

### 서버 실행

\`\`\`bash
cd ~/repos/3rd/mcp_agent_mail
source .venv/bin/activate
python -m mcp_agent_mail.cli serve-http
\`\`\`
```


### 참고 자료 {#참고-자료}

-   [Beads GitHub](https://github.com/steveyegge/beads)
-   [MCP Agent Mail GitHub](https://github.com/Dicklesworthstone/mcp_agent_mail)
-   ~/repos/3rd/beads/docs/AGENT_MAIL.md
-   ~/repos/3rd/beads/docs/DAEMON.md

